...
IReadOnlyList<P22CombatBenchmarkResult> results = P22ReleaseTargets.RunCombatBenchmarks(samples);
var markdown = new StringBuilder()
    .AppendLine("# P22 六构筑空间战斗审计").AppendLine()
    .AppendLine($"每套构筑样本数：{samples}。全部调用正式 `P4SpatialCombatRunner`，固定种子可复现。").AppendLine()
    .AppendLine("| 构筑 | 阶段 | 胜利 | 成功率 | 平均节点时长 |")
    .AppendLine("|---|---|---:|---:|---:|");
foreach (P22CombatBenchmarkResult result in results)
    markdown.AppendLine($"| {result.DisplayName} | {(result.Endgame ? "T20/Boss" : "T1")} | " +
        $"{result.Victories}/{result.Samples} | {result.SuccessRate:P1} | {result.AverageDurationSeconds:F2}s |");
