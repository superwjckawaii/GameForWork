...
$placementPath = Join-Path $docRoot 'Builds_PASSIVE_TREE_PLACEMENT.md'
$sourceFiles = @(
    'Builds_PASSIVE_CLUSTERS.md',
    'Builds_DAMAGE_AILMENT_CLUSTERS.md',
    'Builds_DEFENSE_RESOURCE_CLUSTERS.md',
    'Builds_UNIT_CLUSTERS.md',
    'Builds_SKILL_MECHANISM_CLUSTERS.md',
    'Builds_AUXILIARY_GENERAL_CLUSTERS.md',
    'Builds_VIRTUE_VICE_CLUSTERS.md',
    'Builds_PASSIVE_TREE_TOPOLOGY.md'
)
...
foreach ($line in $placement) {
    $normalized = Normalize-Markdown $line
    if ($normalized -notmatch '^\|\s*((?:V[0-5]-[IRO]\d\d)|(?:E[0-5]-N\d\d))\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|$') {
        continue
    }
    $slot = $Matches[1].Trim()
    $name = $Matches[2].Trim()
...
if (($clusters | Where-Object size -eq 'medium').Count -ne 131 -or
    ...
    throw 'Builds cluster size totals are invalid.'
}
...
$output = Join-Path $RepositoryRoot 'src/Game.Core/Builds/Data/builds-passive-tree.json'
