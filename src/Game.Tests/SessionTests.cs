...
public sealed class SessionTests
{
    ...
    {
        GameSession whole = CreateSession();
        GameSession segmented = CreateSession();
    ...
    {
        GameSessionSnapshot legacy = CreateSession().Capture() with { FormatVersion = 1 };

        InvalidDataException error = Assert.Throws<InvalidDataException>(() => GameSession.Restore(legacy));
        Assert.Contains($"expected {GameSession.CurrentFormatVersion}", error.Message);
    }

    private static GameSession CreateSession() => GameSession.CreateNew(Identity("阿斯特"), 1234);
