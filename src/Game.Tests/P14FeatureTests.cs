...
public sealed class P14FeatureTests
{
    ...
    {
        Assert.Equal(10, P1Skills.All.Count);
        Assert.Equal(18, Enum.GetValues<SkillSupport>().Count(value => value != SkillSupport.None));
        Assert.Equal(18, P2SkillStones.All.Count(item => item.Kind == SkillStoneKind.Support));
        Assert.Equal(40, P1ItemBases.All.Count);
