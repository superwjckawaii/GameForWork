...
public sealed class EquipmentCatalogTests
{
    ...
    public void AtomicCraftingHidesRandomResultAndConsumesPrefixProtectionOnlyOnSuccess()
    {
        ItemInstance rare = Enumerable.Range(0, 200)
            .Select(seed => ItemGenerator.Generate("core.base.life_ring", 100, ItemRarity.Rare, (ulong)seed + 1, "equipment-craft"))
    ...
    {
        int atTwentyOne = ActiveSkillCatalog.Interpolate(100, 300, 21, false);
        int atForty = ActiveSkillCatalog.Interpolate(100, 300, 40, false);
        Assert.True(atForty > atTwentyOne);
        Assert.Equal(atForty, ActiveSkillCatalog.Interpolate(100, 300, 41, false));
        Assert.True(ActiveSkillCatalog.Interpolate(100, 300, 40, true) > ActiveSkillCatalog.Interpolate(100, 300, 21, true));
    }
    ...
    private static AffixDefinition Affix(ItemBaseDefinition itemBase, string family, int tier) =>
        Affixes.For(itemBase, 100).Single(value => value.StableFamilyId == family && value.Tier == tier);
