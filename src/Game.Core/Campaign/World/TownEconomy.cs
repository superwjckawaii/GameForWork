...
public sealed class TownEconomyState
{
    ...
    {
        if (Gold < TownWorkshop.GoldCost || IronScraps < TownWorkshop.IronScrapCost)
        {
            return false;
        }

        Gold -= TownWorkshop.GoldCost;
        IronScraps -= TownWorkshop.IronScrapCost;
        return true;
    }

    public bool TryExchangeLegendary(string stableId, out ItemInstance? legendary)
    {
        UniqueDefinition? selected = LegendaryDrops.ExchangePool
            .FirstOrDefault(item => item.StableId == stableId);
        if (selected is null || WardenMarks < LegendaryDrops.PityMarkCost)
        {
            legendary = null;
            return false;
        }
    ...
    public bool TryExchangeLegendary(out ItemInstance? legendary) =>
        TryExchangeLegendary(LegendaryDrops.ExchangePool[0].StableId, out legendary);
...
public static class TownWorkshop
{
