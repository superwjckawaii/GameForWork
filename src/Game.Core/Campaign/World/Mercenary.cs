...
public sealed class MercenaryProfile
{
    ...
    public TeamBuild CreateTeamBuild(int level = 1)
    {
        SkillConfiguration heavyStrike = AutonomousConfiguration.Skills.First(
            skill => skill.SkillId == SkillIds.HeavyStrike);
        ...
        return new TeamBuild(
            ...
            UseWarCry: AutonomousConfiguration.Skills.Any(skill => skill.SkillId == SkillIds.WarCry),
...
public static class StartingMercenaryFactory
{
    ...
    public static MercenaryProfile GenerateCantor(ulong seed)
    {
        ...
        var configuration = new MercenaryAutonomousConfiguration(
            [
                ...
                new SkillConfiguration(SkillIds.WarCry, SkillSupport.None),
            ],
            "敌群接近后先战吼；保持生命安全时使用扩大范围重击；不读取隐藏掉落结果。",
            "颂仪者倾向：自主提升战吼覆盖与群体清理，玩家不可修改技能、辅助或 AI。");
        return new MercenaryProfile(
