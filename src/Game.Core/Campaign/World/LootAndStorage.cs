...
{
    public static int Compare(ItemInstance left, ItemInstance right, ItemSortMode mode)
    {
        int primary = mode switch
        {
            ItemSortMode.LinkedSockets => right.LinkedSocketCount.CompareTo(left.LinkedSocketCount),
            ItemSortMode.Rarity => right.Rarity.CompareTo(left.Rarity),
            ItemSortMode.ItemLevel => right.ItemLevel.CompareTo(left.ItemLevel),
            ItemSortMode.Base => string.Compare(left.Base.DisplayName, right.Base.DisplayName, StringComparison.Ordinal),
            ItemSortMode.HighestAffixTier => BestTier(left).CompareTo(BestTier(right)),
            _ => 0,
        };
        if (primary != 0) return primary;
        int secondary = mode == ItemSortMode.LinkedSockets
    ...
    private static int BestTier(ItemInstance item) => item.Affixes
        .Select(affix => Affixes.TierFor(item.Base, affix.Definition)).DefaultIfEmpty(int.MaxValue).Min();
}
    ...
    bool RequireGameplayBiasedBase = false)
{
    public bool Matches(ItemInstance item)
    {
        if (!Enabled || Rarity is not null && item.Rarity != Rarity ||
            ...
            BaseTier is not null && DropCatalog.ResolveBaseTier(item.Base) != BaseTier ||
            DropSource is not null && !item.DropSource.Contains(DropSource, StringComparison.OrdinalIgnoreCase) ||
            RequireGameplayBiasedBase && !DropCatalog.IsGameplayBiased(item) ||
            ...
            MinimumEstimatedValue is not null && ItemValue.Estimate(item) < MinimumEstimatedValue ||
            MaximumEstimatedValue is not null && ItemValue.Estimate(item) > MaximumEstimatedValue ||
        ...
        return item.Affixes.Any(affix =>
            ...
            (MinimumAffixTier is null || Affixes.TierFor(item.Base, affix.Definition) >= MinimumAffixTier) &&
            (MaximumAffixTier is null || Affixes.TierFor(item.Base, affix.Definition) <= MaximumAffixTier));
    }
}
...
public sealed class EquipmentStorage
{
    ...
    public void Sort(ItemSortMode mode) => _items.Sort((left, right) => ItemSorting.Compare(left, right, mode));

    public void SortByLinkedSockets() => Sort(ItemSortMode.LinkedSockets);
...
public sealed class ExpeditionBackpack
{
    ...
    public void Replace(IEnumerable<ItemInstance> items)
    {
        ...
        ItemInstance[] replacement = items.Select(SocketRules.Ensure).ToArray();
