...
public sealed record PassiveBuildModifiers(
    ...
    bool DefenseMastery,
    bool WarCryMastery,
    PassiveModifiers? Advanced = null);

public sealed record PassiveModifiers(
    int Physique,
    int Dexterity,
    int Spirit,
    int Energy,
...
{
    public const float LayoutExtent = PassiveTreeCatalog.LayoutExtent;

    private static readonly IReadOnlyDictionary<string, PassiveNodeDefinition> NodeMap = PassiveTreeCatalog.Build()
    ...
    public static string StartNode(PassiveStartKind start) => PassiveTreeCatalog.StartNode(start);

    public static IReadOnlyList<PassiveEffect> MasteryOptions(PassiveNodeDefinition node) =>
        PassiveTreeCatalog.MasteryOptions(node);

    public static IReadOnlyList<string> MasteryOptionDescriptions(PassiveNodeDefinition node) =>
        PassiveTreeCatalog.MasteryOptionDescriptions(node);
        ...
        IReadOnlySet<string>? connectedSources = null)
    {
        ...
        IEnumerable<string> sources = usableSources.Count == 0
            ? new[] { PassiveTreeCatalog.StartNode(start) }
            : usableSources;
        ...
        string root = PassiveTreeCatalog.StartNode(start);
    ...
    public static int AllocatedNodesInRadius(string socketId, IReadOnlySet<string> allocated, float radius = PassiveTreeCatalog.JewelRadius)
    {
...
public sealed class PassiveTreeAllocation
{
    ...
    public bool TryAllocate(string stableId, int earnedPassivePoints, JewelState? jewelState = null)
    {
        ...
        string root = PassiveTreeCatalog.StartNode(StartKind);
        IReadOnlySet<string> connected = RootConnectedAllocated();
        if (!PassiveTree.Neighbors(root).Contains(stableId) &&
            !PassiveTree.Neighbors(stableId).Any(connected.Contains) &&
            !JewelCatalog.GrantsUnlinkedAllocation(jewelState, _allocated, stableId))
        {
            return false;
        }
    ...
    public bool TryAllocatePath(string stableId, int earnedPassivePoints, JewelState? jewelState = null)
    {
        if (JewelCatalog.GrantsUnlinkedAllocation(jewelState, _allocated, stableId))
            return TryAllocate(stableId, earnedPassivePoints, jewelState);
        IReadOnlySet<string> connected = RootConnectedAllocated();
        IReadOnlyList<string> path = PassiveTree.FindShortestPath(stableId, _allocated, StartKind, connected);
    ...
    public bool TryRefund(string stableId, JewelState? jewelState = null)
    {
    ...
    public bool CanRefundWithoutDisconnecting(string stableId, JewelState? jewelState = null)
    {
        ...
        return remaining.All(id => reachable.Contains(id) ||
            JewelCatalog.GrantsUnlinkedAllocation(jewelState, remaining, id, stableId));
    }

    public bool IsValidAllocation(JewelState? jewelState = null, string? ignoredSocket = null)
    {
        HashSet<string> reachable = RootConnected(_allocated);
        return _allocated.All(id => reachable.Contains(id) ||
            JewelCatalog.GrantsUnlinkedAllocation(jewelState, _allocated, id, ignoredSocket));
    }
    ...
    private HashSet<string> RootConnected(IReadOnlySet<string> nodes)
    {
        ...
        string root = PassiveTreeCatalog.StartNode(StartKind);
        var pending = new Queue<string>(PassiveTree.Neighbors(root).Where(nodes.Contains));
        while (pending.TryDequeue(out string? current))
        {
            if (!reachable.Add(current)) continue;
            foreach (string neighbor in PassiveTree.Neighbors(current).Where(nodes.Contains)) pending.Enqueue(neighbor);
        }
        return reachable;
    }
        ...
        JewelState? buildsJewels = null)
    {
        ...
        if (!result.IsValidAllocation(buildsJewels))
            throw new InvalidDataException("Passive allocation snapshot is not a valid path.");
    ...
    public PassiveBuildModifiers CalculateModifiers()
    {
        ...
        void Accumulate(PassiveEffect effect)
        {
            ...
            sums[index] = effect.Kind == PassiveEffectKind.MoreDamageBasisPoints
                ? CombatRules.CombineMoreBasisPoints(sums[index], effect.Value)
                : checked(sums[index] + effect.Value);
        }
        foreach (PassiveEffect effect in _allocated
                     .Select(PassiveTree.Get)
        ...
        {
            PassiveEffect effect = PassiveTree.MasteryOptions(PassiveTree.Get(id))[option];
            Accumulate(effect);
        }
        foreach ((string socket, PassiveJewelKind jewel) in _socketedJewels)
        {
            int radiusMultiplier = 1 + PassiveTree.AllocatedNodesInRadius(socket, _allocated) / 5;
