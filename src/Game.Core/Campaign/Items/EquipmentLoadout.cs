...
public sealed class EquipmentLoadout
{
    ...
    public static EquipmentLoadout Restore(IEnumerable<KeyValuePair<EquipmentSlot, ItemInstance>> items)
    {
        ...
        {
            if (!result.TryEquip(slot, SocketRules.Ensure(item)))
            {
    ...
    private static int LocalMoreValue(ItemInstance item, ItemModifierKind kind)
    {
        ...
        IEnumerable<int> values = item.EffectiveImplicitComponents
            ...
            .Concat(item.CorruptionComponents
                .Where(effect => effect.Kind == kind && effect.Scope == ItemModifierScope.LocalDefense)
                .Select(effect => effect.Value));
        return values.Aggregate(0, CombatRules.CombineMoreBasisPoints);
    }

    private static void AddGlobal(int[] sums, ItemModifierKind kind, int value, ItemModifierScope scope)
    {
        if (kind == ItemModifierKind.None || scope is ItemModifierScope.LocalWeapon or ItemModifierScope.LocalDefense or ItemModifierScope.LocalBlock or ItemModifierScope.Flask)
            return;
        sums[(int)kind] = IsMoreModifier(kind)
            ? CombatRules.CombineMoreBasisPoints(sums[(int)kind], value)
