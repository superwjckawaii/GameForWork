...
public sealed record AssembledCharacterBuild(
    ...
    IReadOnlyList<FlaskKind> Flasks,
    WeaponProfile EffectiveWeapon,
    VirtueViceLoadout? VirtueViceLoadout = null,
...
public static class CharacterBuildAssembler
{
        ...
        JewelState? jewelState = null)
    {
        ...
        WeaponProfile weapon = equipment.Weapon ?? Weapons.Unequipped;
        ...
        JewelModifiers attributeMemory = jewelState is null
            ? new()
            : JewelCatalog.CalculateAttributeMemoryModifiers(jewelState, attributes);
        ...
        heavyStrike = LegendaryRules.ApplyToHeavyStrike(heavyStrike, equipment.WeaponLegendaryRule);
        ...
        return new AssembledCharacterBuild(
            ...
            CombatRules.CombineMoreBasisPoints(jewel.MoreAttackDamageBasisPoints,
                equipment.Effects?.Value(ItemModifierKind.MoreAttackDamageBasisPoints) ?? 0),
            CombatRules.CombineMoreBasisPoints(jewel.MoreSpellDamageBasisPoints,
            ...
            (advanced.Flaskless ? [] : loadout.Items.Where(pair => pair.Key is >= EquipmentSlot.Flask1 and <= EquipmentSlot.Flask5)
                .Select(pair => FlaskRules.KindForBase(pair.Value.Base.StableId)).Where(kind => kind.HasValue)
    ...
    private static VirtueViceLoadout VirtueVice(EquipmentModifiers item, JewelModifiers jewel)
    {
        var maximum = new Dictionary<VirtueViceKind, int>(jewel.AdditionalVirtueViceMaximum ??
            new Dictionary<VirtueViceKind, int>());
        var held = new HashSet<VirtueViceKind>();
        Add(VirtueViceKind.Mercy, ItemModifierKind.MercyMaximum, ItemModifierKind.HoldMercyAtMaximum);
        Add(VirtueViceKind.Temperance, ItemModifierKind.TemperanceMaximum, ItemModifierKind.HoldTemperanceAtMaximum);
        Add(VirtueViceKind.Humility, ItemModifierKind.HumilityMaximum, ItemModifierKind.HoldHumilityAtMaximum);
        Add(VirtueViceKind.Rage, ItemModifierKind.RageMaximum, ItemModifierKind.HoldRageAtMaximum);
        Add(VirtueViceKind.Sloth, ItemModifierKind.SlothMaximum, ItemModifierKind.HoldSlothAtMaximum);
        Add(VirtueViceKind.Arrogance, ItemModifierKind.ArroganceMaximum, ItemModifierKind.HoldArroganceAtMaximum);
        return new(maximum, held.Order().ToArray(), jewel.Oaths ?? []);
        void Add(VirtueViceKind kind, ItemModifierKind maximumKind, ItemModifierKind heldKind)
        {
    ...
    private static int WeaponPassiveIncrease(EquipmentLoadout loadout, PassiveBuildModifiers legacy, PassiveModifiers passive)
    {
