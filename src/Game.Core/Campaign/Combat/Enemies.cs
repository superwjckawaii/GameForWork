...
public sealed record EnemyProfile(
    ...
    int AilmentAvoidanceBasisPoints = 0, int ReducedAilmentDurationBasisPoints = 0, int SpiritBarrier = 0)
{
    public IReadOnlyList<EnemySkillProfile> EffectiveSkills => Skills is { Count: > 0 }
        ? Skills
        : [MonsterCatalog.DefaultSkill(Skill, Family, AttackRangeRaw)];
}
...
public static class Enemies
{
    ...
    public static IReadOnlyList<EnemyProfile> NormalEnemies { get; } = LegacyEnemies
        .Select(MonsterCatalog.EnrichLegacy)
...
public static class EnemyRules
{
        ...
        EnemyRarity? rarity = null)
    {
        ...
        if (monsterLevel >= 70)
        {
            ...
            CombatRarity buildsRarity = rarity.Value switch
            {
            ...
            ResistanceProfile resistance = CombatRules.MonsterResistances(mapTier, buildsRarity);
            ...
            armor = CombatRules.MonsterArmor(mapTier, buildsRarity);
        }
        ...
        return new ScaledEnemy(profile, monsterLevel, rarity.Value, life, minimumDamage, maximumDamage, armor, evasion,
            ...
            Math.Clamp(fireResistance, CombatRules.MinimumResistance, 9_000),
            Math.Clamp(coldResistance, CombatRules.MinimumResistance, 9_000),
            Math.Clamp(lightningResistance, CombatRules.MinimumResistance, 9_000),
            Math.Clamp(voidResistance, CombatRules.MinimumResistance, 9_000),
            Math.Clamp(physicalResistance, CombatRules.MinimumResistance, 5_000));
    }
