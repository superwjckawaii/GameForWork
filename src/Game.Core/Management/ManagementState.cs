...
public sealed record SkillStoneDefinition(
    ...
    SkillCapability Capabilities = SkillCapability.None,
    SkillCapability RequiredAllCapabilities = SkillCapability.None,
    SkillCapability RequiredAnyCapabilities = SkillCapability.None,
    SkillCapability ExcludedCapabilities = SkillCapability.None,
    SkillSupport CombatSupport = SkillSupport.None,
    SupportMechanic ArchetypeSupport = SupportMechanic.None,
    SupportConflict ProvidesConflict = SupportConflict.None,
    SupportConflict ConflictsWith = SupportConflict.None,
    string SupportId = "");

public static class SkillStoneCatalog
{
    private static readonly IReadOnlyDictionary<string, SkillStoneDefinition> Catalog =
        ActiveSkillCatalog.Active.Select(Active)
            .Concat(ActiveSkillCatalog.Supports.Select(Support))
    ...
    private static SkillStoneDefinition Active(ActiveSkillDefinition definition) =>
    ...
    private static SkillStoneDefinition Support(SupportSkillDefinition definition) =>
        new(definition.StoneId, definition.DisplayName, SkillStoneKind.Support, 1,
            ...
            ArchetypeSupport: definition.LegacyArchetypesSupport, ProvidesConflict: definition.ProvidesConflict,
            ConflictsWith: definition.ConflictsWith, SupportId: definition.StoneId);
}
...
public sealed record SkillSchemeSnapshot(SkillSchemeKind Kind, IReadOnlyList<SkillLinkConfiguration> Links);
public sealed record SchemeSwitchResult(bool Succeeded, int MissingStones, int EjectedStones, string Message);
...
public sealed record ManagementSnapshot(
    ...
    IReadOnlyList<SkillSchemeSnapshot>? SkillSchemes = null,
    SkillSchemeKind ActiveSkillScheme = SkillSchemeKind.Clear,
...
public sealed class ManagementState
{
    ...
    private readonly Dictionary<SkillSchemeKind, IReadOnlyList<SkillLinkConfiguration>> _skillSchemes = [];
    ...
    public SkillSchemeKind ActiveSkillScheme { get; private set; } = SkillSchemeKind.Clear;
    public string PreviewSkillStoneInstanceId { get; private set; } = string.Empty;
    public IReadOnlyDictionary<SkillSchemeKind, IReadOnlyList<SkillLinkConfiguration>> SkillSchemes => _skillSchemes;

    public static ManagementState CreateNew(BaseClass baseClass = BaseClass.Fighter)
    {
        var state = new ManagementState();
        foreach (SkillStoneDefinition definition in SkillStoneCatalog.All.Where(item => item.StarterGranted)
        ...
        state._skillLinks.Add(new SkillLinkConfiguration(heavyStrike, [bleed], 1, SkillChainIds.WeaponPrimary));
        state._skillLinks.Add(new SkillLinkConfiguration(earthCleave, [], 2, SkillChainIds.WeaponSecondary));
        state._skillLinks.Add(new SkillLinkConfiguration(spiritBlade, [chain], 3, SkillChainIds.Chest));
        state._skillLinks.Add(new SkillLinkConfiguration(warCry, [], 4, SkillChainIds.HelmetTool));
        state.ConfigureStarterSkill(baseClass);
        state.SaveAllSchemesFromCurrent();
        return state;
    }

    private void ConfigureStarterSkill(BaseClass baseClass)
    {
        if (baseClass == BaseClass.Fighter) return;
        ...
        if (stone is null)
        {
            _ = SkillStoneCatalog.Get(stoneId);
            stone = new SkillStoneInstance($"starter-characters-{definition.StableId.Split('.').Last()}", stoneId);
            _skillStones.Add(stone);
        }
        SkillLinkConfiguration primary = _skillLinks.Single(link => link.ChainId == SkillChainIds.WeaponPrimary);
    ...
    public static ManagementState Restore(ManagementSnapshot? snapshot, bool legacyMigration)
    {
        ManagementState state = snapshot is null ? CreateNew() : new ManagementState();
        if (snapshot is not null)
        {
            if (snapshot.SkillStones.Any(s => s.Quality is < 0 or > 20 || s.Level is < 1 or > 20))
                throw new InvalidDataException("技能石品质或等级无效。");
            state._sortingBag.AddRange(snapshot.SortingBag.Take(SortingBagCapacity).Select(SocketRules.Ensure));
            state._recovery.AddRange(snapshot.Recovery.Select(SocketRules.Ensure));
            state._recovery.AddRange(snapshot.SortingBag.Skip(SortingBagCapacity).Select(SocketRules.Ensure));
            state._buyback.AddRange(snapshot.Buyback.TakeLast(BuybackCapacity)
                .Select(entry => entry with { Item = SocketRules.Ensure(entry.Item) }));
            ...
            foreach (SkillSchemeSnapshot scheme in snapshot.SkillSchemes ?? [])
            {
        ...
        foreach (SkillStoneDefinition definition in SkillStoneCatalog.All.Where(definition => definition.StarterGranted &&
        ...
        state.EnsureSpatialSkillLink("core.skill_stone.earth_cleave", 2);
        state.EnsureSpatialSkillLink("core.skill_stone.spirit_blade", 3, "core.skill_stone.chain");
        state.EnsureSpatialSkillLink("core.skill_stone.war_cry", 4);
    ...
    private void EnsureSpatialSkillLink(string activeDefinitionId, int priority, string? supportDefinitionId = null)
    {
    ...
    public ManagementSnapshot Capture() => new(
        _sortingBag.ToArray(),
        _recovery.ToArray(),
        _buyback.ToArray(),
        ...
        _skillSchemes.Select(pair => new SkillSchemeSnapshot(pair.Key, CloneLinks(pair.Value))).ToArray(),
        ActiveSkillScheme,
    ...
    public bool SelectPreviewSkill(string stoneInstanceId)
    {
        ...
        if (stone?.Definition.Kind != SkillStoneKind.Active ||
            !stone.Definition.Capabilities.HasFlag(SkillCapability.Damage) ||
            ActiveSkillCatalog.ActiveForStone(stone.DefinitionId).Combat.Role is
    ...
    public void AddToRecovery(ItemInstance item, string reason)
    {
        ...
        {
            throw new InvalidOperationException($"Item {item.InstanceId} already exists in a Management management container.");
        }
    ...
    public bool TryAssignActiveToChain(
        string activeStoneInstanceId,
        string chainId,
        IReadOnlyList<SkillChainDefinition> chains)
    {
        SkillStoneInstance? active = _skillStones.FirstOrDefault(item => item.InstanceId == activeStoneInstanceId);
        SkillChainDefinition? chain = chains.FirstOrDefault(item => item.StableId == chainId);
        if (active is null || chain is null || !SkillChainRules.Accepts(chain, active.Definition))
        {
            return false;
        }
    ...
    public void SortSortingBag(ItemSortMode mode) =>
        _sortingBag.Sort((left, right) => ItemSorting.Compare(left, right, mode));
        ...
        IReadOnlyList<SkillChainDefinition> chains)
    {
        ...
        if (stone.Definition.Kind == SkillStoneKind.Active && installedSupports.Any(support =>
                !SkillCompatibility.Check(stone.Definition, support).Compatible))
        {
            return false;
        }
        if (active is not null && stone.Definition.Kind == SkillStoneKind.Support &&
            !SkillCompatibility.CheckGroup(active.Definition, stone.Definition, installedSupports).Compatible)
        {
            return false;
        }
    ...
    public bool UnsocketStone(string chainId, int socketIndex, IReadOnlyList<SkillChainDefinition> chains)
    {
        SkillChainDefinition? chain = chains.FirstOrDefault(item => item.StableId == chainId);
    ...
    public int NormalizeSkillChains(IReadOnlyList<SkillChainDefinition> chains)
    {
        ...
        foreach (SkillLinkConfiguration original in _skillLinks.OrderBy(item => item.Priority).ToArray())
        {
            ...
            string chainId = valid.Contains(original.ChainId) && !occupied.Contains(original.ChainId)
                ...
                : string.Empty;
            SkillChainDefinition? chain = chains.FirstOrDefault(item => item.StableId == chainId);
            if (chain is null || occupied.Contains(chainId) || active is not null && !SkillChainRules.Accepts(chain, active.Definition))
            {
                chain = chains.FirstOrDefault(candidate => !occupied.Contains(candidate.StableId) &&
                    (active is null || SkillChainRules.Accepts(candidate, active.Definition)));
    ...
    public void SaveSkillScheme(SkillSchemeKind kind)
    {
    ...
    public SchemeSwitchResult SwitchSkillScheme(
        SkillSchemeKind kind,
        IReadOnlyList<SkillChainDefinition> chains)
    {
        if (!_skillSchemes.TryGetValue(kind, out IReadOnlyList<SkillLinkConfiguration>? saved))
        {
            return new SchemeSwitchResult(false, 0, 0, "技能方案不存在。");
        }
        ...
        return new SchemeSwitchResult(true, missing, ejected,
    ...
    public static int SalePrice(ItemInstance item) => Economy.ItemValue.SalePrice(item);
    ...
    private IReadOnlySet<string> EffectiveInstalledStoneIds()
    {
                     ...
                     !string.IsNullOrEmpty(link.ActiveStoneInstanceId)))
        {
            ...
            foreach (string supportId in link.SupportStoneInstanceIds)
            {
                ...
                if (support is not null && SkillCompatibility.Check(active.Definition, support.Definition).Compatible)
                {
    ...
    {
        foreach (SkillSchemeKind kind in Enum.GetValues<SkillSchemeKind>())
        {
            _skillSchemes[kind] = CloneLinks(_skillLinks);
        }
    }
