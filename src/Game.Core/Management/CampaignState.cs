...
public sealed record CampaignSnapshot(
    ...
    SceneTimeline? ActiveTimeline = null);

public sealed record CampaignAdvanceResult(
...
public sealed class CampaignState
{
    ...
    public SceneTimeline? ActiveTimeline { get; private set; }
    ...
    public CampaignNodeDefinition? CurrentNode => Completed ? null : CampaignCatalog.Nodes[CurrentNodeIndex];
    public int CurrentAct => Completed ? 5 : CurrentNode?.Act ?? 1;

    public static CampaignState CreateNew()
    {
    ...
    public static CampaignState CreateLegacyCompleted()
    {
        var state = new CampaignState
        {
            CurrentNodeIndex = CampaignCatalog.Nodes.Count,
            Completed = true,
        };
        state._completedNodeIds.UnionWith(CampaignCatalog.Nodes.Select(node => node.StableId));
        state._claimedRewardNodeIds.UnionWith(state._completedNodeIds);
        state.AddLog("旧存档迁移：五幕主线已标记完成，远征保持开放。");
        return state;
    }

    public static CampaignState Restore(CampaignSnapshot? snapshot, bool legacyMigration)
    {
        if (snapshot is null)
        {
        ...
        CampaignNodeDefinition? currentNode = snapshot.CurrentNodeIndex >= 0 &&
            snapshot.CurrentNodeIndex < CampaignCatalog.Nodes.Count
            ? CampaignCatalog.Nodes[snapshot.CurrentNodeIndex]
            : null;
        HashSet<string> validIds = CampaignCatalog.Nodes.Select(node => node.StableId).ToHashSet(StringComparer.Ordinal);
        if (snapshot.CurrentNodeIndex < 0 || snapshot.CurrentNodeIndex > CampaignCatalog.Nodes.Count ||
            ...
            snapshot.Completed != (snapshot.CurrentNodeIndex == CampaignCatalog.Nodes.Count) ||
            ...
            CampaignCatalog.Nodes.Take(snapshot.CurrentNodeIndex)
    ...
    public CampaignSnapshot Capture() => new(
        CurrentNodeIndex,
    ...
    internal void BeginTimeline(SceneTimeline timeline)
    {
    ...
    internal void CompleteCurrentNode(bool rewardClaimed)
    {
        ...
        ActiveTimeline = null;
        Defeated = false;
        if (CurrentNodeIndex >= CampaignCatalog.Nodes.Count)
        {
            CurrentNodeIndex = CampaignCatalog.Nodes.Count;
            Completed = true;
        ...
        {
            AddLog($"第 {CurrentNode.Act} 幕 · {CampaignCatalog.ActNames[CurrentNode.Act - 1]} 开始。");
        }
    }
...
public sealed class CampaignSimulator
{
    ...
    private sealed record TimelinePreparation(string NodeId, int BuildHash, Task<SceneTimeline> Task);
    private TimelinePreparation? _timelinePreparation;
    public CampaignAdvanceResult Simulate(
        ...
        bool asyncPreparation = false)
    {
        ...
        while (remaining > 0 && !campaign.Completed && !campaign.Defeated)
        {
            ...
            else
            {
                ...
                {
                    TeamBuild currentBuild = world.Hero.Build with
                    {
                    ...
                    if (asyncPreparation)
                    {
                        ...
                        if (_timelinePreparation?.NodeId != node.StableId || _timelinePreparation.BuildHash != buildHash)
                        {
                            _timelinePreparation = new TimelinePreparation(node.StableId, buildHash,
                                Task.Run(() => SceneTimelineBuilder.BuildCampaign(currentBuild, node, nodeSeed)));
                        }
                        ...
                        Task<SceneTimeline> completed = _timelinePreparation.Task;
                    ...
                    {
                        _timelinePreparation = null;
                        campaign.BeginTimeline(SceneTimelineBuilder.BuildCampaign(currentBuild, node, nodeSeed));
                    }
                }
            ...
            if (campaign.ActiveTimeline is not null &&
                campaign.ActiveTimeline.Outcome != BattleOutcome.HeroVictory)
            {
                world.Expedition.AddCombatReport(CombatReportBuilder.Build(
        ...
        return new CampaignAdvanceResult(
            effective,
    ...
    public bool Replay(
        ...
        ulong seed)
    {
        CampaignNodeDefinition node = CampaignCatalog.Get(stableId);
        if (!campaign.CompletedNodeIds.Contains(stableId) || node.Kind == CampaignNodeKind.StoryEvent)
        {
            return false;
        }

        SceneTimeline replay = SceneTimelineBuilder.BuildCampaign(world.Hero.Build, node, seed);
        world.Expedition.AddCombatReport(CombatReportBuilder.Build(replay, $"主线重放 · {node.DisplayName}"));
        if (replay.Outcome != BattleOutcome.HeroVictory)
        {
    ...
    private static void GrantDefeatedEnemyRewards(
        ...
        ulong seed)
    {
        int defeated = DropFormula.ExtractDefeated(timeline, CampaignLevels.MonsterLevel(node)).Count;
        ...
        ulong seed)
    {
        ...
        {
            world.MapInventory.Add(new MapItem($"campaign-map-{node.Act}-{node.IndexInAct}", Math.Min(10, node.Act - 3)));
        }
    ...
    private static void GrantCombatLoot(WorldState world, ManagementState management,
        CampaignNodeDefinition node, SceneTimeline timeline, ulong seed, bool completed)
    {
        int monsterLevel = CampaignLevels.MonsterLevel(node);
        IReadOnlyList<DefeatedEnemy> defeated = DropFormula.ExtractDefeated(timeline, monsterLevel);
        var context = new LootContext(node.StableId, monsterLevel, 10_000,
            0,
            MapRoute.Safe, AllowMaps: false, Completed: completed,
            BossPool: node.Kind == CampaignNodeKind.ActBoss ? "campaign" : string.Empty);
        RewardBatch rewards = DropFormula.Roll(context, defeated, seed);
    ...
    private static string Hash(CampaignState campaign, WorldState world, long elapsed, ulong seed)
    {
