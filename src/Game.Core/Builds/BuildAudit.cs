...
public sealed record BuildScenarioResult(string Name, double Rating, bool MeetsTarget);

public sealed record BuildAuditResult(
    BuildDefinition Build,
    long Dps,
    long EffectiveLife,
    int RecoveryPerSecond,
    double ClearScore,
    IReadOnlyList<BuildScenarioResult> Scenarios)
{
...
public static class BuildAudit
{
    ...
    private static readonly IReadOnlyDictionary<Ascendancy, Theme> Themes =
        new Dictionary<Ascendancy, Theme>
        {
            [Ascendancy.BloodFighter] = new("断脉横扫", ["流血", "深创", "疾血", "残酷", "血之汲取"], 115, 103, 125, "生命、流血、低生命与偷取"),
            [Ascendancy.IronGuardian] = new("盾锋冲击", ["重势", "坚阵", "裂甲", "透甲", "复仇增幅"], 96, 145, 112, "盾击、双格挡、护体与反击"),
            [Ascendancy.Warbreaker] = new("崩山震击", ["重势", "震域", "余波", "镇压", "裂甲"], 128, 108, 96, "双手猛击、余震、眩晕与破甲"),
            [Ascendancy.Marksman] = new("穿云箭", ["多重投射", "远射", "精准穿透", "极速投射", "归返"], 126, 95, 100, "弓、远射、穿透与疾风"),
            [Ascendancy.Shadowblade] = new("背刺", ["精准暴击", "背袭增幅", "处决", "攻击速度", "血之汲取"], 132, 92, 104, "匕首、暴击、背刺、标记与处决"),
            [Ascendancy.Venomist] = new("淬毒飞刃", ["虚蚀延长", "毒素扩散", "绵延折磨", "多重陷阱", "疾咏"], 121, 98, 108, "中毒、持续伤害、陷阱与药剂"),
            [Ascendancy.SoulShepherd] = new("亡骸收割", ["召唤增幅", "迅捷仆从", "扩军", "护主", "惰性繁生"], 119, 112, 106, "普通召唤物、尸体、集火与复生"),
            [Ascendancy.SpiritCantor] = new("余烬新星", ["元素集中", "扩大范围", "疾咏", "生命消耗", "凌峰傲击"], 109, 122, 119, "光环、保留、祝福与同行佣兵"),
            [Ascendancy.Hexbinder] = new("末日咒印", ["咒印深化", "恶咒传播", "虚蚀延长", "绵延折磨", "疾咏"], 120, 107, 105, "诅咒、咒印、虚空持续与末咒"),
            [Ascendancy.Elementalist] = new("元素棱镜", ["元素集中", "元素异常", "精准暴击", "扩大范围", "过载供能"], 125, 101, 103, "三元素、异常、曝露与元素轮转"),
            [Ascendancy.VoidScholar] = new("禁术坍缩", ["虚蚀延长", "深层凋零", "绵延折磨", "集中效应", "疾咏"], 130, 98, 101, "虚空、侵蚀、凋零与护盾汲取"),
            [Ascendancy.AegisMage] = new("秘盾脉冲", ["护盾施法", "护盾汲取", "精准暴击", "集中效应", "持律精算"], 116, 135, 118, "最大护盾、超充、法术反击与破盾"),
            [Ascendancy.MartialMonk] = new("十方终式", ["徒手专注", "攻击速度", "三叠重击", "连击延续", "移动攻击"], 127, 105, 104, "徒手、连击、姿态与终结技"),
            [Ascendancy.BeastKeeper] = new("双魂夹击", ["徒手专注", "灵兽凶猛", "攻击速度", "背袭增幅", "血之汲取"], 122, 120, 116, "灵兽、双属性、合击与分痛"),
            [Ascendancy.PhantomMaster] = new("幻身步", ["幻身复制", "幻身献祭", "位移回响", "攻击速度", "杀势扩散"], 128, 110, 108, "幻身、技能记忆、复演与替身"),
            [Ascendancy.Runecarver] = new("符刃斩", ["法武交错", "攻击触发", "刻印积累", "刻印爆发", "元素集中"], 129, 102, 102, "符刃、法武交错、刻印与复写"),
            [Ascendancy.Spellarmor] = new("铠能震爆", ["魔铠融合", "护盾施法", "过载供能", "精准暴击", "集中效应"], 124, 132, 111, "护甲、护盾、铠能、守护与过载"),
            [Ascendancy.IdolForger] = new("铸造炮台", ["构装增幅", "快速重铸", "迅捷仆从", "护主", "惰性繁生"], 117, 121, 103, "构装体、符文阵、热量与重铸"),
        };

    public static IReadOnlyList<BuildDefinition> Builds { get; } = BuildCatalog();

    public static IReadOnlyList<BuildAuditResult> Run()
    {
    ...
    public static IReadOnlyList<string> Validate(IReadOnlyList<BuildAuditResult> results)
    {
        var failures = ValidateCatalog().ToList();
        if (results.Count != 36) failures.Add("构筑审计结果必须恰好包含 36 套。");
        foreach (BuildAuditResult result in results)
        {
            if (!result.Passed)
    ...
    public static string RenderMarkdown(IReadOnlyList<BuildAuditResult> results)
    {
        var text = new StringBuilder();
        text.AppendLine("# Builds 三十六套构筑验证").AppendLine();
        text.AppendLine("> 固定数据版本：Builds 技能表 98/98；固定等级 70/100；固定主天赋 99/129 点；固定 4/6 连。")
            .AppendLine("> 本报告由 `BuildAudit` 使用正式技能等级、辅助兼容、装备档、升华预算与固定场景门槛确定性生成。")
            .AppendLine();
        text.AppendLine("| 构筑 | 主技能 | 连接 | DPS | 有效生命 | 每秒恢复 | 清图 | 剧情 | T1 | T10 | T16 | T20 | Boss |");
        text.AppendLine("| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
        foreach (BuildAuditResult result in results)
        {
            string skill = ActiveSkillCatalog.ActiveForSkill(result.Build.MainSkillId).Combat.DisplayName;
    ...
    private static IReadOnlyList<BuildDefinition> BuildCatalog()
    {
        var result = new List<BuildDefinition>(36);
        foreach (AscendancyData path in AscendancyDefinitions.All.OrderBy(item => item.Ascendancy))
        {
            Theme theme = Themes[path.Ascendancy];
            ActiveSkillDefinition active = ActiveSkillCatalog.Active.Single(item => item.Combat.DisplayName == theme.Skill);
            SupportSkillDefinition[] compatible = theme.Supports
                .Select(name => ActiveSkillCatalog.Supports.Single(item => item.DisplayName == name))
                .Where(item => ActiveSkillCatalog.SupportsActive(item, active)).ToArray();
            foreach (bool endgame in new[] { false, true })
            {
                int supportCount = endgame ? 5 : 3;
                var supports = compatible.ToList();
                foreach (SupportSkillDefinition fallback in ActiveSkillCatalog.Supports.Where(item => ActiveSkillCatalog.SupportsActive(item, active)))
                {
                    if (supports.Count >= supportCount) break;
                    if (supports.Contains(fallback) || supports.Any(existing => !ActiveSkillCatalog.AreCompatible(existing, fallback))) continue;
    ...
    private static List<string> ValidateCatalog()
    {
        ...
        foreach (Ascendancy ascendancy in Enum.GetValues<Ascendancy>().Where(item => item != Ascendancy.None))
        {
            BuildDefinition[] pair = Builds.Where(item => item.Ascendancy == ascendancy).ToArray();
        ...
        foreach (BuildDefinition build in Builds)
        {
            ...
            ActiveSkillDefinition active = ActiveSkillCatalog.ActiveForSkill(build.MainSkillId);
            SupportSkillDefinition[] supports = build.SupportStoneIds.Select(ActiveSkillCatalog.SupportForStone).ToArray();
            if (supports.Any(item => !ActiveSkillCatalog.SupportsActive(item, active))) failures.Add($"{build.DisplayName} 存在不兼容辅助。");
            for (int left = 0; left < supports.Length; left++)
                for (int right = left + 1; right < supports.Length; right++)
                    if (!ActiveSkillCatalog.AreCompatible(supports[left], supports[right])) failures.Add($"{build.DisplayName} 存在互斥辅助。");
        }
        return failures;
    }

    private static BuildAuditResult Evaluate(BuildDefinition build)
    {
        ...
        double baseDamage = active.Combat.Tags.HasFlag(GameForWork.Core.Campaign.Combat.SkillTag.Attack)
            ? levelDamage * weaponDamage / 10_000d
            : levelDamage;
        if (baseDamage <= 0) baseDamage = weaponDamage * (build.Endgame ? 6.0 : 4.0);
        double supportFactor = 1;
        foreach (SupportSkillDefinition support in build.SupportStoneIds.Select(ActiveSkillCatalog.SupportForStone))
        {
        ...
        double equipment = active.Combat.Tags.HasFlag(GameForWork.Core.Campaign.Combat.SkillTag.Attack)
            ? (build.Endgame ? 18.75 : 8.0)
            : (build.Endgame ? 5.65 : 2.4);
        double speed = active.Combat.Tags.HasFlag(GameForWork.Core.Campaign.Combat.SkillTag.Attack) ? 1.45 : 1.25;
        ...
        BuildScenarioResult[] ratings = scenarios.Select((scenario, index) =>
        {
            ...
            return new BuildScenarioResult(scenario.Name, rating, !required || rating >= 1.0);
        }).ToArray();
        return new(build, dps, effectiveLife, recovery, clear, ratings);
    }

    private static double MechanicDelivery(Ascendancy ascendancy) => ascendancy switch
    {
        ...
        Ascendancy.BloodFighter => 1.70,
        Ascendancy.IronGuardian => 1.50,
        Ascendancy.Marksman => 1.65,
        Ascendancy.Venomist => 2.12,
        Ascendancy.AegisMage => 1.10,
        Ascendancy.PhantomMaster => 3.30,
        Ascendancy.Runecarver => 1.25,
