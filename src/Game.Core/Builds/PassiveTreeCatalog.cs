...
public sealed record PassiveTreeData(
    string Version,
    int CanvasWidth,
    int CanvasHeight,
    int InnerRadius,
    int MiddleRadius,
    int OuterRadius,
    IReadOnlyList<ClusterData> Clusters);

public sealed record ClusterData(
    string Slot,
    string StableSlug,
    string Name,
    string Theme,
...
public static partial class PassiveTreeCatalog
{
    public const string Version = "builds.v1";
    ...
    private static readonly PassiveTreeData Data = Load();
    ...
    public static IReadOnlyList<PassiveNodeDefinition> Build()
    {
        ...
        if (nodes.Count != ExpectedNodeCount)
            throw new InvalidDataException($"Builds passive catalog produced {nodes.Count} nodes instead of {ExpectedNodeCount}.");
        if (nodes.Select(node => node.StableId).Distinct(StringComparer.Ordinal).Count() != nodes.Count)
            throw new InvalidDataException("Builds passive catalog contains duplicate stable IDs.");
        return nodes;
    }
    ...
    public static string StartNode(PassiveStartKind start)
    {
        ...
        return index >= 0 ? $"builds.start.v{index}" : throw new ArgumentOutOfRangeException(nameof(start));
    }
    ...
    public static IReadOnlyList<MasteryChoice> MasteryChoices(PassiveNodeDefinition node)
    {
        if (node.Kind != PassiveNodeKind.Mastery) return [];
        ClusterData cluster = Data.Clusters.First(item =>
            $"builds.mastery.{item.MasteryKey}" == node.MasteryGroup);
        return cluster.MasteryOptions.Select((description, index) =>
            new MasteryChoice(
                ExplicitMasteryEffects.Get(cluster.MasteryKey, index),
                description,
                $"builds.mastery.rule.{cluster.MasteryKey}.{index}")).ToArray();
    }

    public static string MasteryMechanicId(PassiveNodeDefinition node, int option) =>
        MasteryChoices(node)[option].MechanicId;

    public static int ExplicitMasteryRuleCount => ExplicitMasteryEffects.RuleCount;
        ...
        IDictionary<string, (float X, float Y)> positions)
    {
        ...
        {
            string id = $"builds.start.v{vertex}";
        ...
        bool startsAsVertices)
    {
        ...
        {
            string from = startsAsVertices ? $"builds.start.v{edge}" : VertexId(ring, edge);
            string to = startsAsVertices ? $"builds.start.v{(edge + 1) % 6}" : VertexId(ring, (edge + 1) % 6);
        ...
        IDictionary<string, (float X, float Y)> positions)
    {
        for (int vertex = 0; vertex < 6; vertex++)
        {
            AddRadial(nodes, positions, vertex, "i2m", $"builds.start.v{vertex}", VertexId("middle", vertex),
    ...
    {
        foreach (ClusterData cluster in Data.Clusters.OrderBy(item => item.Slot, StringComparer.Ordinal))
        {
    ...
    private static void AddMediumCluster(ICollection<PassiveNodeDefinition> nodes, ClusterData cluster,
    ...
    private static void AddLargeCluster(ICollection<PassiveNodeDefinition> nodes, ClusterData cluster,
    ...
    private static void AddClusterNode(ICollection<PassiveNodeDefinition> nodes, ClusterData cluster,
    ...
    private static string ClusterId(ClusterData cluster, string suffix) => $"builds.cluster.{cluster.StableSlug}.{suffix}";
    private static string RingId(string ring, int edge, int point) => $"builds.path.ring.{ring}.e{edge}.p{point:00}";
    private static string RadialId(int vertex, string segment, int point) => $"builds.path.radial.v{vertex}.{segment}.p{point:00}";
    private static string VertexId(string ring, int vertex) => $"builds.vertex.{ring}.v{vertex}";
    ...
    private static string NodeName(ClusterData cluster, PassiveNodeKind kind, string description, int ordinal) => kind switch
    {
    ...
    private static void ValidateData()
    {
        if (Data.Version != Version || Data.Clusters.Count != ExpectedMediumClusters + ExpectedLargeClusters ||
            ...
            throw new InvalidDataException("Builds passive-tree resource does not match builds.v1 totals.");
    ...
    {
        const string resource = "GameForWork.Core.Builds.Data.builds-passive-tree.json";
