...
public sealed record ActiveSkillDefinition(
...
{
    public int DamageAt(int level) => ActiveSkillCatalog.Interpolate(LevelOneDamage, LevelTwentyOneDamage, level, Curve != SkillCurve.Linear);
    public int ManaAt(int level) => ActiveSkillCatalog.Interpolate(LevelOneMana, LevelTwentyOneMana, level, false);
}

public sealed record SupportSkillDefinition(
    string StoneId,
    string DisplayName,
    string MechanicKey,
    ...
    SupportMechanic LegacyArchetypesSupport = SupportMechanic.None,
    ...
    bool StarterGranted = false)
{
    public int ValueAt(int level, int quality = 0)
    {
        ...
        return ActiveSkillCatalog.Interpolate(LevelOneValue, LevelTwentyOneValue, level, false);
    }
}

public sealed record SupportRuntimeProfile(
    IReadOnlyList<SupportSkillDefinition> Supports,
    int ResourceMultiplierBasisPoints,
...
public sealed record LinkedSupport(string StoneId, int Level, int Quality);

public static class ActiveSkillCatalog
{
    ...
    public static IReadOnlyList<ActiveSkillDefinition> Active { get; } = BuildActive();
    public static IReadOnlyList<SupportSkillDefinition> Supports { get; } = BuildSupports();
    private static readonly IReadOnlyDictionary<string, ActiveSkillDefinition> ActiveBySkill =
        Active.ToDictionary(item => item.Combat.SkillId, StringComparer.Ordinal);
    private static readonly IReadOnlyDictionary<string, ActiveSkillDefinition> ActiveByStone =
        Active.ToDictionary(item => item.Combat.StoneId, StringComparer.Ordinal);
    private static readonly IReadOnlyDictionary<string, SupportSkillDefinition> SupportByStone =
        Supports.ToDictionary(item => item.StoneId, StringComparer.Ordinal);
    private static readonly IReadOnlyDictionary<SkillSupport, SupportSkillDefinition> SupportByLegacy =
        Supports.Where(item => item.LegacySupport != SkillSupport.None).ToDictionary(item => item.LegacySupport);
    private static readonly IReadOnlyDictionary<SupportMechanic, SupportSkillDefinition> SupportByArchetypes =
        Supports.Where(item => item.LegacyArchetypesSupport != SupportMechanic.None).ToDictionary(item => item.LegacyArchetypesSupport);

    static ActiveSkillCatalog()
    {
        if (Active.Count != 86 || ActiveBySkill.Count != 86 || ActiveByStone.Count != 86)
            throw new InvalidDataException("Builds requires 86 unique active skills including shield bash and five high-reservation auras.");
        if (Supports.Count != 98 || SupportByStone.Count != 98)
            throw new InvalidDataException("Builds requires 98 unique support skills.");
    }

    public static ActiveSkillDefinition ActiveForSkill(string id) => ActiveBySkill.TryGetValue(id, out ActiveSkillDefinition? value)
        ? value : throw new KeyNotFoundException($"Unknown Builds active skill: {id}");
    ...
    public static SupportSkillDefinition SupportFor(SupportMechanic support) => SupportByArchetypes[support];

    public static bool SupportsActive(SupportSkillDefinition support, ActiveSkillDefinition active)
    {
        SkillCapability capabilities = active.Combat.Capabilities;
        return (capabilities & support.RequiredAll) == support.RequiredAll &&
               (support.RequiredAny == SkillCapability.None || (capabilities & support.RequiredAny) != 0) &&
               (capabilities & support.Excluded) == 0;
    }

    public static bool AreCompatible(SupportSkillDefinition left, SupportSkillDefinition right)
    {
    ...
    public static SupportRuntimeProfile ResolveSupports(ActiveSkillDefinition active,
        IEnumerable<string> stoneIds, int level, int quality)
        => ResolveSupports(active, stoneIds.Select(id => new LinkedSupport(id, level, quality)));

    public static SupportRuntimeProfile ResolveSupports(ActiveSkillDefinition active,
        IEnumerable<LinkedSupport> supportLinks)
    {
        LinkedSupport[] links = supportLinks.GroupBy(item => item.StoneId, StringComparer.Ordinal)
            .Select(group => group.First()).ToArray();
        SupportSkillDefinition[] linked = links.Select(item => SupportForStone(item.StoneId)).ToArray();
        foreach (SupportSkillDefinition support in linked)
        ...
        {
            SupportSkillDefinition support = linked[index];
            LinkedSupport link = links[index];
            ...
            switch (support.MechanicKey)
            {
                ...
                case "dot.prolonged":
                    damage = active.Combat.Role == SkillRole.DamageOverTime
    ...
    private static IReadOnlyList<ActiveSkillDefinition> BuildActive()
    {
        SkillCombatDefinition[] legacy = CoreSkillDefinitions.Active
            .Concat(ArchetypeSkillDefinitions.Active.Select(item => item.Combat)).ToArray();
        var result = legacy.Select(active => Convert(active)).ToList();
        result.Add(Convert(NewActive("builds.skill_stone.shield_bash", "builds.skill.shield_bash", "盾锋冲击",
            SkillTag.Attack | SkillTag.Melee | SkillTag.Area | SkillTag.Physical,
            SkillCapability.Damage | SkillCapability.Hit | SkillCapability.Attack |
            SkillCapability.Melee | SkillCapability.Area | SkillCapability.PhysicalDamage |
            SkillCapability.CanCrit | SkillCapability.CanStun | SkillCapability.RequiresShield,
            SkillRole.SingleTarget, SkillDamageType.Physical, SkillShape.Cone, 10, 2_200, 0, 0, 12_000,
            "盾牌防御换算点伤的主动盾击。"), 12_000, 35_000, 15, "范围效果提高 20%"));
        result.AddRange(new[]
        {
    ...
    private static ActiveSkillDefinition Convert(SkillCombatDefinition active,
        int? oneOverride = null, int? twentyOneOverride = null, int? manaTwentyOneOverride = null, string quality = "品质效果见技能说明")
    {
        string buildsName = active.DisplayName switch
        {
            "镜式反击" => "镜雷反制",
            "魔铠过载" => "铠能震爆",
            "破盾反击" => "碎盾回震",
        ...
        (int Cast, int Cooldown, int Range)? timing = active.SkillId switch
        {
            ...
            "archetypes.skill.chain_fists" => (7, 0, 1_500),
            "archetypes.skill.skyquake_palm" => (12, 0, 3_500),
            "archetypes.skill.gale_kick" => (11, 20, 5_000),
            "archetypes.skill.tenfold_finisher" => (16, 40, 4_000),
            "archetypes.skill.twin_soul_pincer" => (12, 22, 6_000),
        ...
        return new(active, curve, one, twentyOne, active.ManaCost, mana21, quality);
    }

    private static ActiveSkillDefinition Aura(string suffix, string name, int reservationPercent, string description)
    {
        SkillCombatDefinition active = NewActive($"builds.skill_stone.{suffix}", $"builds.skill.{suffix}", name,
            ...
            SkillRole.Reservation, SkillDamageType.None, SkillShape.Self, reservationPercent, 10_000, 0, 0, 0, description);
        return Convert(active, 0, 0, reservationPercent, "范围或对应核心效果提高 20%");
    }

    private static SkillCombatDefinition NewActive(string stone, string skill, string name, SkillTag tags,
        SkillCapability capabilities, SkillRole role, SkillDamageType damageType, SkillShape shape,
        ...
        new(stone, skill, name, tags, capabilities, role, damageType, shape, mana, range, cast, cooldown, damage,
            Ailment.None, 0, description);

    private static SkillCurve Curve(SkillCombatDefinition active)
    {
        if (active.DamageBasisPoints == 0) return SkillCurve.Linear;
        if (active.Role == SkillRole.DamageOverTime) return SkillCurve.DamageOverTime;
        if (active.Tags.HasFlag(SkillTag.Attack))
            return active.DisplayName is "连环拳" or "震空掌" or "追风踢" or "十方终式" or "双魂夹击"
                ? SkillCurve.UnarmedAttack : SkillCurve.WeaponAttack;
        if (active.Tags.HasFlag(SkillTag.Spell)) return SkillCurve.HitSpell;
        return SkillCurve.Linear;
    }

    private static int DefaultTwentyOne(int one, SkillCurve curve) => curve switch
    {
        SkillCurve.WeaponAttack => checked((int)Math.Round(one * Math.Pow(1.05, 20))),
        SkillCurve.UnarmedAttack or SkillCurve.Unit => checked((int)Math.Round(one * Math.Pow(1.06, 20))),
        SkillCurve.ShieldAttack => checked((int)Math.Round(one * Math.Pow(1.055, 20))),
        SkillCurve.HitSpell => checked((int)Math.Round(one * Math.Pow(1.07, 20))),
        SkillCurve.DamageOverTime => checked((int)Math.Round(one * Math.Pow(1.065, 20))),
        _ => one,
    };

    private static IReadOnlyList<SupportSkillDefinition> BuildSupports()
    {
        var result = CoreSkillDefinitions.Supports.Select(Convert).Concat(ArchetypeSkillDefinitions.Supports.Select(Convert)).ToList();
        SkillCapability cost = SkillCapability.HasCost;
        SkillCapability hit = SkillCapability.Hit | SkillCapability.Damage;
        result.AddRange(new[]
        {
            NewSupport("mercy_expansion", "仁心扩界", "virtue.mercy", SkillCapability.None, SkillCapability.Area | SkillCapability.Duration | SkillCapability.Guard, SkillCapability.None, 15, 25, 12_000, "每层慈悲强化范围、持续、恢复与守护。", "每层范围再提高 3%"),
            NewSupport("temperance_calculus", "持律精算", "virtue.temperance", cost, SkillCapability.None, SkillCapability.Reservation, 4, 6, 10_000, "每层节制使资源消耗总降，并使主动技能有效等级 +1、品质提高 5%。", "每层资源消耗再总降 1%"),
            NewSupport("humility_guard", "俯身守式", "virtue.humility", SkillCapability.None, SkillCapability.Attack | SkillCapability.Spell | SkillCapability.WarCry, SkillCapability.Triggerable, 3, 5, 11_500, "动作期间每层谦逊提供击中总降与眩晕防护。", "动作后保护延长至 1 秒"),
            NewSupport("rage_acceleration", "怒潮催行", "vice.rage", SkillCapability.None, SkillCapability.Attack | SkillCapability.Spell | SkillCapability.WarCry, SkillCapability.Triggerable, 6, 10, 12_500, "每层暴怒使动作速度总增，技能造成 20% 更少伤害。", "更少伤害降至 15%"),
            NewSupport("sloth_proliferation", "惰性繁生", "vice.sloth", SkillCapability.None, SkillCapability.Projectile | SkillCapability.Duration, SkillCapability.None, 10, 18, 13_000, "每层懒惰强化投射物、陷阱、普通召唤物与持续时间。", "每层持续时间再提高 5%"),
            NewSupport("arrogance_critical", "凌峰傲击", "vice.arrogance", SkillCapability.Hit | SkillCapability.CanCrit, SkillCapability.None, SkillCapability.None, 6, 10, 13_000, "每层傲慢强化暴击率与暴击更多伤害，非暴击造成更少伤害。", "非暴击更少伤害降至 15%"),
            NewSupport("lone_focus", "孤锋专注", "target.single", hit, SkillCapability.None, SkillCapability.None, 40, 60, 14_000, "每次使用最多伤害一个敌人，禁用多目标弹道和溅射。", "对稀有与 Boss 再造成 10% 更多伤害"),
            NewSupport("kill_spread", "杀势扩散", "kill.explosion", hit, SkillCapability.None, SkillCapability.None, 35, 55, 13_000, "击杀产生保留原伤害类型的爆炸，每次使用最多三次。", "爆炸范围提高 20%"),
            NewSupport("prolonged_torment", "绵延折磨", "dot.prolonged", SkillCapability.Duration | SkillCapability.Damage, SkillCapability.None, SkillCapability.None, 30, 50, 13_000, "持续伤害更多且持续更久，击中造成 25% 更少伤害。", "持续伤害生效速度提高 10%"),
            NewSupport("overload_supply", "过载供能", "cycle.overload", SkillCapability.Hit | SkillCapability.HasCost, SkillCapability.Attack | SkillCapability.Spell, SkillCapability.Channelling | SkillCapability.Triggerable, 20, 50, 15_000, "积蓄两次，第三次以 200% 消耗过载复行一次。", "复行延迟降至 0.10 秒"),
        });
        return result;
    }

    private static SupportSkillDefinition Convert(SupportCompatibilityDefinition legacy)
    {
    ...
    private static SupportSkillDefinition Convert(ArchetypeSupportDefinition legacy)
    {
        (int one, int twentyOne, int cost, string quality) = Tuning(legacy.DisplayName);
        return new(legacy.StoneId, legacy.DisplayName, MechanicKey(legacy.DisplayName), legacy.RequiredAll,
            legacy.RequiredAny, legacy.Excluded, one, twentyOne, cost,
            $"{legacy.Description} 1～21级核心数值 {one}%→{twentyOne}%。", quality,
            LegacyArchetypesSupport: legacy.Mechanic);
    }

    private static SupportSkillDefinition NewSupport(string suffix, string name, string key,
        SkillCapability all, SkillCapability any, SkillCapability excluded, int one, int twentyOne,
        int cost, string effect, string quality) =>
        new($"builds.skill_stone.support.{suffix}", name, key, all, any, excluded, one, twentyOne, cost, effect, quality);
