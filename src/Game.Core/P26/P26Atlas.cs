...
public sealed record P26AtlasNode(
    string StableId,
    string DisplayName,
    P26AtlasCategory Category,
    int Position,
    int GoldCost,
    P26AtlasGate Gate,
    string Effect,
...
public static class P26AtlasCatalog
{
    ...
    private static readonly (P26AtlasCategory Category, string Prefix, string CategoryName, string[] Names, string[] Effects, int[] Optional)[] Source =
    [
        (P26AtlasCategory.MapBasics, "map", "地图基础",
        ...
        (P26AtlasCategory.Supply, "supply", "地图续航",
        ...
        (P26AtlasCategory.Crafting, "craft", "地图打造",
        ...
        (P26AtlasCategory.PacksAndElites, "pack", "怪群精英",
        ...
        (P26AtlasCategory.Boss, "boss", "Boss攻坚",
        ...
        (P26AtlasCategory.Abyss, "abyss", "深渊",
        ...
        (P26AtlasCategory.LifeGarden, "garden", "命能花园",
        ...
        (P26AtlasCategory.RedOath, "red", "赤誓祭坛",
        ...
        (P26AtlasCategory.BlueOath, "blue", "苍誓祭坛",
        ...
        (P26AtlasCategory.Warfront, "warfront", "战阵前线",
            ["战功增产", "前线装备", "前线金属", "四重军需", "军官奖励", "军需偏向", "指挥官", "声望增产", "军官底材", "前线碎片", "扩大战线", "决战前线"],
            ["战功获取提高 50%", "战阵装备数量提高 50%", "战阵金属获取提高 50%", "军需奖励选项增加到 4 个", "军官奖励提高 80%", "可选择武器、护甲、首饰或材料作为军需偏向", "指挥官出现权重提高 50%", "战阵声望获取提高 100%", "军官掉落底材权重提高 100%", "战阵 Boss 碎片掉落提高 100%", "可选：战线增加 2 个节点，军官数量翻倍，奖励 150% 更多", "可选：战线增加 4 个节点；敌人 150% 更多生命、90% 更多伤害，禁用救援，奖励 300% 更多，并替代扩大战线"], [11, 12]),
    ];

    private static readonly IReadOnlyList<P26AtlasNode> Nodes = Build();
    private static readonly IReadOnlyDictionary<string, P26AtlasNode> ById = Nodes.ToDictionary(node => node.StableId, StringComparer.Ordinal);
    public static IReadOnlyList<P26AtlasNode> All => Nodes;
    public static P26AtlasNode Get(string id) => ById.TryGetValue(id, out P26AtlasNode? node)
        ? node : throw new KeyNotFoundException($"Unknown P26 atlas node: {id}");

    public static bool GateSatisfied(P26AtlasNode node, int maximumCompletedTier, bool finalBreakthroughCompleted,
        bool warfrontDiscovered)
    {
        if (node.HiddenUntilWarfront && !warfrontDiscovered) return false;
        return node.Gate switch
        {
            P26AtlasGate.Act5 => true,
            P26AtlasGate.Tier5 => maximumCompletedTier >= 5,
            P26AtlasGate.Tier10 => maximumCompletedTier >= 10,
            P26AtlasGate.Tier16 => maximumCompletedTier >= 16,
            P26AtlasGate.FinalBreakthrough => finalBreakthroughCompleted,
            P26AtlasGate.Tier20 => maximumCompletedTier >= 20,
            _ => false,
        };
    }

    private static IReadOnlyList<P26AtlasNode> Build()
    {
        var result = new List<P26AtlasNode>(MaximumNodes);
        foreach ((P26AtlasCategory category, string prefix, string _, string[] names, string[] effects, int[] optional) in Source)
        {
            for (int index = 0; index < 12; index++)
            {
                int position = index + 1;
                P26AtlasGate gate = position switch
                {
                    <= 3 => P26AtlasGate.Act5,
                    <= 6 => P26AtlasGate.Tier5,
                    <= 8 => P26AtlasGate.Tier10,
                    <= 10 => P26AtlasGate.Tier16,
                    11 => P26AtlasGate.FinalBreakthrough,
                    _ => P26AtlasGate.Tier20,
                };
                string id = $"p26.atlas.{prefix}.{position:00}";
                result.Add(new P26AtlasNode(id, names[index], category, position, Costs[index], gate, effects[index],
                    position == 1 ? null : $"p26.atlas.{prefix}.{position - 1:00}", optional.Contains(position),
                    category == P26AtlasCategory.Warfront));
            }
        }
        if (result.Count != MaximumNodes || result.Sum(node => node.GoldCost) != TotalGoldCost)
            throw new InvalidOperationException("P26 atlas catalog contract is invalid.");
        return result;
    }
}

public static class P26AtlasPurchase
{
    public static bool TryPurchase(ISet<string> allocated, P26AtlasNode node, TownEconomyState economy,
        int maximumCompletedTier, bool finalBreakthroughCompleted, bool warfrontDiscovered)
    {
        ...
        if (allocated.Contains(node.StableId) || allocated.Count >= P26AtlasCatalog.MaximumNodes ||
            node.PrerequisiteId is not null && !allocated.Contains(node.PrerequisiteId) ||
            !P26AtlasCatalog.GateSatisfied(node, maximumCompletedTier, finalBreakthroughCompleted, warfrontDiscovered) ||
            !economy.TrySpendGold(node.GoldCost)) return false;
        return allocated.Add(node.StableId);
    }
}

public static class P26AtlasEffects
{
    ...
    public static int EquipmentQuantityIncrease(IReadOnlyList<string>? allocated, bool boss) =>
        (Has(allocated, "p26.atlas.map.05") ? 2_000 : 0) + (boss && Has(allocated, "p26.atlas.boss.01") ? 4_000 : 0);

    public static int GoldIncrease(IReadOnlyList<string>? allocated, bool boss) =>
        (Has(allocated, "p26.atlas.map.02") ? 2_000 : 0) + (boss && Has(allocated, "p26.atlas.boss.02") ? 5_000 : 0);

    public static int MetalIncrease(IReadOnlyList<string>? allocated, bool boss) =>
        (Has(allocated, "p26.atlas.map.03") ? 2_000 : 0) + (boss && Has(allocated, "p26.atlas.boss.02") ? 5_000 : 0);

    public static int SkillStoneIncrease(IReadOnlyList<string>? allocated, bool boss) =>
        (Has(allocated, "p26.atlas.map.06") ? 4_000 : 0) + (boss && Has(allocated, "p26.atlas.boss.09") ? 10_000 : 0);

    public static int EquipmentRarityIncrease(IReadOnlyList<string>? allocated, EnemyRarity source) =>
        (Has(allocated, "p26.atlas.map.07") ? 4_000 : 0) +
        (source == EnemyRarity.Magic && Has(allocated, "p26.atlas.pack.05") ? 5_000 : 0) +
        (source == EnemyRarity.Rare && Has(allocated, "p26.atlas.pack.06") ? 8_000 : 0);

    public static int MapQuantityIncrease(IReadOnlyList<string>? allocated, bool boss) =>
        (Has(allocated, "p26.atlas.supply.01") ? 2_000 : 0) +
        (Has(allocated, "p26.atlas.supply.05") ? 3_000 : 0) +
        (Has(allocated, "p26.atlas.supply.06") ? 3_000 : 0) +
        (boss && Has(allocated, "p26.atlas.boss.03") ? 5_000 : 0);

    public static int EnemyBudgetIncrease(IReadOnlyList<string>? allocated, EnemyRarity rarity) => rarity switch
    {
        EnemyRarity.Normal when Has(allocated, "p26.atlas.pack.01") => 2_000,
        EnemyRarity.Magic when Has(allocated, "p26.atlas.pack.02") => 4_000,
        EnemyRarity.Rare when Has(allocated, "p26.atlas.pack.03") => 6_000,
        _ => 0,
    };

    public static int ExperienceIncrease(IReadOnlyList<string>? allocated) => Has(allocated, "p26.atlas.map.01") ? 2_000 : 0;
    public static int MapSaleIncrease(IReadOnlyList<string>? allocated) => Has(allocated, "p26.atlas.craft.04") ? 5_000 : 0;
}
