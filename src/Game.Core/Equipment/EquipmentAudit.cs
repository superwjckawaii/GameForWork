...
public sealed record EquipmentAuditResult(
    int SampleCount,
...
public static class EquipmentAudit
{
    ...
    public static EquipmentAuditResult Run(int sampleCount = RequiredSampleCount)
    {
        if (sampleCount < RequiredSampleCount)
            throw new ArgumentOutOfRangeException(nameof(sampleCount), $"Equipment audit requires at least {RequiredSampleCount:N0} items.");
        ...
        for (int index = 0; index < sampleCount; index++)
        {
            ...
            ItemInstance item = ItemGenerator.Generate(itemBase.StableId, itemLevel, rarity, seed, $"equipment-audit-{index:D6}");
        ...
        for (int index = 0; index < Math.Min(2_048, sampleCount); index++)
        {
            ...
            ItemInstance first = ItemGenerator.Generate(itemBase.StableId, itemLevel, rarity, SeedFor(index), $"equipment-audit-{index:D6}");
            ItemInstance second = ItemGenerator.Generate(itemBase.StableId, itemLevel, rarity, SeedFor(index), $"equipment-audit-{index:D6}");
            if (Signature(first) != Signature(second)) AddFailure(failures, $"#{index}: repeated generation is not deterministic");
        }

        return new EquipmentAuditResult(sampleCount, coveredBases.Count, bases.Length, observedFamilies.Count,
    ...
    private static void ValidateCatalogReachability(IReadOnlyList<ItemBaseDefinition> bases, List<string> failures)
    {
        ...
        {
            ItemInstance item = EquipmentLegendaryFactory.Create(legendary.Id, 100, $"equipment-legendary-audit-{index}", (ulong)index + SeedSalt);
