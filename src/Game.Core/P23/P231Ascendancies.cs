...
public sealed record P231BranchSpec(
    string StableKey,
...
public sealed record P231PathSpec(
    P18Ascendancy Ascendancy,
    string StableKey,
    IReadOnlyList<P231BranchSpec> Branches);

public static class P231NodeIds
{
...
public static class P231AscendancyCatalog
{
    private static P231BranchSpec B(string key, string smallName, string smallEffect, string coreName, string coreEffect) =>
        new(key, smallName, smallEffect, coreName, coreEffect);

    public static IReadOnlyList<P231PathSpec> Paths { get; } =
    [
        new(P18Ascendancy.Marksman, "marksman",
        [
        ...
        new(P18Ascendancy.Shadowblade, "shadowblade",
        [
        ...
        new(P18Ascendancy.Venomist, "venomist",
        [
        ...
        new(P18Ascendancy.SoulShepherd, "soul_shepherd",
        [
        ...
        new(P18Ascendancy.SpiritCantor, "spirit_cantor",
        [
        ...
        new(P18Ascendancy.Hexbinder, "hexbinder",
        [
        ...
        new(P18Ascendancy.Elementalist, "elementalist",
        [
        ...
        new(P18Ascendancy.VoidScholar, "void_scholar",
        [
        ...
        new(P18Ascendancy.AegisMage, "aegis_mage",
        [
        ...
        new(P18Ascendancy.MartialMonk, "martial_monk",
        [
        ...
        new(P18Ascendancy.BeastKeeper, "beast_keeper",
        [
        ...
        new(P18Ascendancy.PhantomMaster, "phantom_master",
        [
        ...
        new(P18Ascendancy.Runecarver, "runecarver",
        [
        ...
        new(P18Ascendancy.Spellarmor, "spellarmor",
        [
            B("hybrid", "双层甲胄", "护甲和最大护盾提高20%", "魔铠一体", "护甲的30%同时转化为最大护盾；护盾的30%参与物理减伤"),
            B("charge", "受击蓄能", "受到击中获得1层铠能，最多10层，每层护甲提高3%", "蓄能魔铠", "每层同时使法术伤害提高4%；施法消耗全部层数并造成每层8%更多伤害"),
            B("absorb", "铠能吸收", "护盾承受伤害时回复所受伤害5%的生命", "逆流装甲", "回复提高至15%；满生命时改为生成等量屏障"),
            B("overload", "热机", "护盾高于50%时攻击和施法速度提高15%", "魔铠过载", "主动消耗50%当前护盾进入过载6秒，攻击和法术造成50%更多伤害"),
            B("break", "裂甲电弧", "护盾耗尽时对附近敌人造成护盾上限30%的伤害", "破盾反击", "伤害提高至100%，必定眩晕普通敌人并为Boss施加5层破甲，冷却6秒"),
            B("guard", "奥术护甲", "护卫技能同时获得最大护盾15%的屏障", "永动壁甲", "护卫期间受到伤害会补充铠能，达到10层时刷新护卫持续时间，每次护卫限1次"),
        ]),
        new(P18Ascendancy.IdolForger, "idol_forger",
        [
    ...
    public static IReadOnlyList<P18AscendancyNode> Nodes { get; } = Build();

    public static string Id(P18Ascendancy ascendancy, string branch, P18NodeKind kind)
    {
        P231PathSpec path = Paths.Single(item => item.Ascendancy == ascendancy);
        return $"core.ascendancy.{path.StableKey}.{branch}.{(kind == P18NodeKind.Core ? "core" : "small")}";
    }

    private static IReadOnlyList<P18AscendancyNode> Build()
    {
        var nodes = new List<P18AscendancyNode>(180);
        foreach (P231PathSpec path in Paths)
        {
            ...
            {
                P231BranchSpec branch = path.Branches[direction];
                ...
                nodes.Add(new(smallId, path.Ascendancy, direction, P18NodeKind.Reinforcement,
                    branch.ReinforcementName, branch.ReinforcementEffect, null, smallX, smallY));
                nodes.Add(new(coreId, path.Ascendancy, direction, P18NodeKind.Core,
...
public enum P231PrimaryElement { Fire, Cold, Lightning }

public readonly record struct P231ProjectileProfile(
...
public static class P231AscendancyRules
{
    public static P231ProjectileProfile Projectile(P18CombatProfile profile) =>
        profile.Has(P231NodeIds.MarksmanMultipleCore)
            ? new(2, 3_000, 6_500, true)
            : new(0, 0, 10_000, false);

    public static P6ResolvedSkill ApplyResolvedSkill(P6ResolvedSkill skill, SkillTag tags, P18CombatProfile profile)
    {
        if (!tags.HasFlag(SkillTag.Projectile)) return skill;
        P231ProjectileProfile projectile = Projectile(profile);
        ...
        return skill with
        {
            ...
            ProjectileSpeedRawPerSecond = P231ModifierMath.ApplyIncreased(
                skill.ProjectileSpeedRawPerSecond, projectile.IncreasedProjectileSpeedBasisPoints),
            DamageMultiplierBasisPoints = P231ModifierMath.ApplyMore(
                skill.DamageMultiplierBasisPoints, projectile.MoreProjectileDamageBasisPoints),
        };
    }

    public static int MaximumMinions(int externalAdditionalMaximum, P18CombatProfile profile) => checked(
        P23CombatLimits.MaximumMinions + Math.Max(0, externalAdditionalMaximum) +
        (profile.Has(P231NodeIds.SoulLegionCore) ? 2 : 0));

    public static int IncreasedMinionDamageBasisPoints(int livingMinions, P18CombatProfile profile) =>
        profile.Has(P231NodeIds.SoulLegionCore) ? checked(Math.Max(0, livingMinions - 8) * 1_500) : 0;

    public static P231AuraProfile Aura(P18CombatProfile profile) => new(
        profile.Has(P231NodeIds.CantorReservationCore) ? 6_000 : 10_000,
        profile.Has(P231NodeIds.CantorAuraCore) ? 5_000 : 0,
        profile.Has(P231NodeIds.CantorBlessingCore) ? 1 : 0);

    public static int ExtraPhysicalAsPrimaryElement(int originalPhysicalDamage, P231PrimaryElement element,
        P18CombatProfile profile)
    {
        _ = element;
        return profile.Has(P231NodeIds.ElementalistConversionCore)
            ? checked(Math.Max(0, originalPhysicalDamage) * 5_000 / 10_000)
            : 0;
    }

    public static P231EnergyShieldProfile EnergyShield(P18CombatProfile profile) => new(
        profile.Has(P231NodeIds.AegisMaximumCore) ? 13_000 : 10_000,
        profile.Has(P231NodeIds.AegisRechargeCore) ? 20 : P23EnergyShieldState.RechargeDelayTicks,
        profile.Has(P231NodeIds.AegisRechargeCore) ? 5_000 : 0);

    public static bool ConstructPrioritizes(EnemyRarity rarity, P18CombatProfile profile) =>
        profile.Has(P231NodeIds.IdolTurretCore) && rarity is EnemyRarity.Rare or EnemyRarity.Boss;

    public static int ConstructDamageMultiplier(EnemyRarity rarity, P18CombatProfile profile) =>
        ConstructPrioritizes(rarity, profile) ? 13_000 : 10_000;
}

public sealed class P231MechanicRuntime(P18CombatProfile profile)
{
...
public static class P231BenchmarkBuilds
{
    public static IReadOnlyList<P18BenchmarkBuild> All { get; } = P231AscendancyCatalog.Paths
        .SelectMany(path => new[]
        {
    ...
    private static P18BenchmarkBuild Create(P231PathSpec path, bool endgame, IReadOnlyList<int> directions)
    {
        string mode = endgame ? "endgame" : "entry";
        string displayMode = endgame ? "终局" : "开荒";
        string[] nodes = directions.SelectMany(direction => new[]
        {
            P231AscendancyCatalog.Id(path.Ascendancy, path.Branches[direction].StableKey, P18NodeKind.Reinforcement),
            P231AscendancyCatalog.Id(path.Ascendancy, path.Branches[direction].StableKey, P18NodeKind.Core),
        }).ToArray();
        return new($"core.benchmark.{path.StableKey}.{mode}",
            $"{P18AscendancyCatalog.DisplayName(path.Ascendancy)}·{displayMode}", path.Ascendancy, endgame,
