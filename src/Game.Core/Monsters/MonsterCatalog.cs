...
namespace GameForWork.Core.Monsters;

public sealed record MonsterFamilyDefinition(
    EnemyFamily Family,
    string StableId,
    string DisplayName,
    string Identity,
...
{
    public static IReadOnlyList<MonsterFamilyDefinition> Families { get; } =
    [
        new(EnemyFamily.AshenLegion, "monsters.family.ashen_legion", "烬骸军团", "物理与火焰、冲锋和战旗", EnemyDamageType.Fire),
        new(EnemyFamily.FrostwildPack, "monsters.family.frostwild_pack", "霜原猎群", "冰霜、包围和冰缓区域", EnemyDamageType.Cold),
        new(EnemyFamily.DrownedDead, "monsters.family.drowned_dead", "沉墓亡者", "尸体利用、骨矢和钟波", EnemyDamageType.Physical),
        new(EnemyFamily.BloodforgeConstruct, "monsters.family.bloodforge_construct", "血炉造物", "高护甲、熔炉和修复构装", EnemyDamageType.Lightning),
        new(EnemyFamily.VoidCult, "monsters.family.void_cult", "虚空教团", "诅咒、裂隙地面和召唤", EnemyDamageType.Void),
        new(EnemyFamily.RiftBeast, "monsters.family.rift_beast", "裂渊异兽", "钻地、追猎和递增强度", EnemyDamageType.Void),
        new(EnemyFamily.LifeGarden, "monsters.family.life_garden", "命能孽种", "缠根、孢子和治疗花", EnemyDamageType.Cold),
        new(EnemyFamily.RedOath, "monsters.family.red_oath", "赤誓刑团", "流血、献祭和处决", EnemyDamageType.Fire),
        new(EnemyFamily.BlueOath, "monsters.family.blue_oath", "苍誓星侍", "延迟法术、护盾链接和雷霜", EnemyDamageType.Lightning),
        new(EnemyFamily.Warfront, "monsters.family.warfront", "亡旗军阵", "盾墙、压制齐射和军官号令", EnemyDamageType.Physical),
    ];
    ...
    static MonsterCatalog()
    {
        if (Families.Count != 10 || Families.Select(item => item.Family).Distinct().Count() != 10)
            throw new InvalidDataException("Monsters requires ten distinct monster families.");
        if (AdditionalEnemies.Count != 32 || AdditionalEnemies.Select(item => item.StableId).Distinct().Count() != 32)
            throw new InvalidDataException("Monsters requires thirty-two new non-boss monsters.");
    }
    ...
    public static EnemyFamily FamilyForEncounter(string areaId, MapNodeKind? nodeKind, MapAltar altar, int nodeIndex, ulong seed)
    {
        if (nodeKind == MapNodeKind.AbyssFissure) return EnemyFamily.RiftBeast;
        if (nodeKind == MapNodeKind.GardenPlot) return EnemyFamily.LifeGarden;
        if (nodeKind is MapNodeKind.WarfrontEncounter or MapNodeKind.WarfrontOfficer or MapNodeKind.WarfrontCommander)
            return EnemyFamily.Warfront;
        if (nodeKind == MapNodeKind.Altar)
            return altar == MapAltar.RedOath ? EnemyFamily.RedOath : EnemyFamily.BlueOath;
        ...
        params EnemySkillProfile[] skills)
    {
        ...
        return new EnemyProfile($"monsters.enemy.{id}", name, life, min, max, armor, evasion, accuracy, speed, attacks,
