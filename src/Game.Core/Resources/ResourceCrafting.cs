...
public sealed record ResourceCraftResult(bool Succeeded, string Summary, ItemInstance? Result, int Cost);

public static class ResourceCrafting
{
    public const int RedFavorCost = 100;
    public const int BlueFavorCost = 100;

    public static ResourceCraftResult ShiftAffixTier(ItemInstance item, string affixFamilyId, ulong seed)
    {
        ...
        AffixDefinition[] family = Affixes.For(item.Base, item.ItemLevel)
            ...
            .OrderBy(definition => Affixes.TierFor(item.Base, definition)).ToArray();
        if (family.Length < 2) return Fail("该词缀在当前底材上没有可升降档位。", RedFavorCost);
        int currentTier = Affixes.TierFor(item.Base, current.Definition);
        var random = new Pcg32(seed);
        int direction = random.NextBasisPoints() < 5_000 ? -1 : 1;
        int desired = currentTier + direction;
        AffixDefinition? target = family.FirstOrDefault(definition => Affixes.TierFor(item.Base, definition) == desired);
        if (target is null)
        {
            direction = -direction;
            desired = currentTier + direction;
            target = family.FirstOrDefault(definition => Affixes.TierFor(item.Base, definition) == desired);
        }
    ...
    public static ResourceCraftResult RerollQuality(ItemInstance item, ulong seed)
    {
    ...
    private static ResourceCraftResult Fail(string summary, int cost) => new(false, summary, null, cost);
}
