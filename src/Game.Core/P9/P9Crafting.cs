...
public static class P9CraftingRules
{
    ...
    private static P9CraftResult Divine(ItemInstance item, ulong seed)
    {
        ...
        var random = new Pcg32(seed);
        AffixRoll[] rerolled = item.Affixes.Select(affix => affix.Crafted || item.IsFractured(affix) ? affix : affix with
        {
            Value = RollInclusive(random, affix.Definition.MinimumValue, affix.Definition.MaximumValue),
        }).ToArray();
