...
internal static class UiText
{
    ...
    public static string ItemTooltip(ItemInstance item, bool includeAffixDetails = false)
    {
        ...
        text.AppendLine($"底材阶级：{DropCatalog.BaseTierName(DropCatalog.ResolveBaseTier(item.Base))}");
        string[] requirements =
        [
        ...
        {
            UniqueDefinition? unique = UniqueItems.All.FirstOrDefault(definition =>
                definition.StableId == item.LegendaryRule.StableId);
            if (unique is not null)
            {
                foreach (LegendaryAffixDefinition affix in unique.LegendaryAffixes)
        ...
        if (FlaskRules.KindForBase(item.Base.StableId) is { } flaskKind)
        {
            FlaskDefinition flask = Flasks.All.Single(definition => definition.Kind == flaskKind);
        ...
        text.AppendLine();
        text.Append($"出售 {ItemValue.SalePrice(item)} 金币");
    ...
    public static string PassiveTooltip(
        ...
        bool available)
    {
        if (node.StableId.StartsWith("builds.", StringComparison.Ordinal))
        {
            ...
            string effects = node.Kind == PassiveNodeKind.Mastery
                ? "从以下专精中选择一项：\n" + string.Join('\n',
                    PassiveTree.MasteryOptionDescriptions(node).Select((text, index) => $"{index + 1}. {text}"))
                : !string.IsNullOrWhiteSpace(node.SpecialRule)
                    ? node.SpecialRule
                    : string.Join('\n', node.Effects.Select(PassiveEffect));
            string buildsJewel = node.Kind == PassiveNodeKind.JewelSocket
    ...
    public static string JewelTooltip(JewelInstance jewel)
    {
        string affixes = string.Join('\n', jewel.Affixes.Select(affix =>
            $"{JewelCatalog.PositionName(affix.Position)} · T{affix.Tier} · {JewelCatalog.AffixText(affix)}"));
