...
public partial class WorldView : Control
{
    ...
    private readonly List<SceneEvent> _recentEvents = [];
    ...
    {
        Vector2 shake = VisualPreferences.ScreenShake ? ScreenShakeOffset() : Vector2.Zero;
    ...
    private void DrawActiveBattle(Rect2 bounds)
    {
        ...
        SceneTimeline? timeline = observed?.Timeline;
        if (timeline is null)
        {
        ...
        EncounterSegment? activeEncounter = timeline.Encounters.FirstOrDefault(segment =>
        ...
        DrawArtSprite(enemyAnimation,
            boss ? ArtContract.BossRig(timeline.StableId) : ArtContract.EnemyRig(timeline.StableId),
            Facing.Left, SpriteAction.Attack, (long)_visualElapsedMilliseconds, enemy,
            boss ? new Vector2(62, 70) : new Vector2(40, 50),
            boss ? ArtContract.BossRigCount : ArtContract.EnemyBodyRigCount,
            boss ? ArtContract.BossCellWidth : ArtContract.ActorCellWidth,
            boss ? ArtContract.BossCellHeight : ArtContract.ActorCellHeight);

        IReadOnlyList<SceneEvent> recent = timeline?.Events
        ...
        {
            string phase = state?.Kind == SceneEventKind.TravelStarted
                ? $"移动中 · {state.Value} 格"
    ...
    private void DrawTravel(Rect2 bounds, ObservedScene observed, SceneEvent? state, long elapsed)
    {
        Rect2 field = new(bounds.Position + new Vector2(18, 54), bounds.Size - new Vector2(36, 82));
        bool moving = state?.Kind == SceneEventKind.TravelStarted;
    ...
    private void DrawSpatialBattle(
        ...
        bool hero)
    {
        IReadOnlyList<SpatialFrame> frames = timeline.SpatialFrames!;
        int frameIndex = FindFrameIndex(frames, elapsed);
        SpatialFrame current = frames[frameIndex];
        SpatialFrame next = frameIndex + 1 < frames.Count ? frames[frameIndex + 1] : current;
        ...
        for (int row = 0; row <= SceneTimeline.LogicalHeight; row++)
        {
            float y = field.Position.Y + field.Size.Y * row / SceneTimeline.LogicalHeight;
        ...
        _nextEnemies.Clear();
        foreach (EnemyFrame enemy in next.Enemies) _nextEnemies[enemy.EntityId] = enemy;
        _positions.Clear();
        _positions["hero"] = actor;
        foreach (EnemyFrame enemy in current.Enemies)
        {
            Point position = _nextEnemies.TryGetValue(enemy.EntityId, out EnemyFrame? future)
        ...
        DrawSpatialSkills(field, actor, _positions,
            SceneEffectPlayback.Select(timeline.Events, elapsed, VisualPreferences.EffectLimit(Size.X)), elapsed);

        foreach (EnemyFrame enemy in current.Enemies.Where(enemy => enemy.Life > 0 ||
                     HasRecentEvent(_recentEvents, SceneEventKind.EnemyDefeated, targetId: enemy.EntityId)))
        {
            ...
            EnemyFrame future = _nextEnemies.GetValueOrDefault(enemy.EntityId) ?? enemy;
            Facing facing = FacingBetween(enemy.Position, future.Position, current.HeroPosition);
            SpriteAction action = ResolveEnemyAction(enemy, future, _recentEvents
                .Where(item => elapsed - item.AtMilliseconds <
                    (EventTargets(item, enemy.EntityId) && IsDamageEvent(item.Kind) ? 180 : 500)).ToArray());
            long actionAge = EventAge(_recentEvents, elapsed, action == SpriteAction.Death
                ? SceneEventKind.EnemyDefeated
                : action == SpriteAction.Hit ? null : SceneEventKind.EnemyAttack, enemy.EntityId);
        ...
        DrawShadow(actor + new Vector2(0, 6), 10);
        EnemyFrame? heroTarget = current.Enemies.FirstOrDefault(enemy => enemy.EntityId == current.HeroTargetId);
        Facing heroFacing = FacingBetween(current.HeroPosition, next.HeroPosition,
            heroTarget?.Position ?? new Point(current.HeroPosition.XRaw, current.HeroPosition.YRaw - 1));
        SpriteAction heroAction = ResolveHeroAction(current, next, _recentEvents.Where(item =>
            elapsed - item.AtMilliseconds < (EventTargets(item, "hero") && IsDamageEvent(item.Kind) ? 180 : 500)).ToArray());
        foreach (AllyFrame ally in current.Allies ?? [])
            DrawSpatialAlly(field, ally, current, next, elapsed, hero);
        SceneEvent? heroActionEvent = timeline.Events.LastOrDefault(item => item.AtMilliseconds <= elapsed &&
            (heroAction == SpriteAction.Death ? item.HeroLife <= 0 :
             heroAction == SpriteAction.Hit ? EventTargets(item, "hero") && IsDamageEvent(item.Kind) :
             EventSource(item, "hero") && IsHeroAction(item.Kind)));
        long heroActionAge = heroAction is SpriteAction.Idle or SpriteAction.Move ? elapsed :
            Math.Max(0, elapsed - (heroActionEvent?.AtMilliseconds ?? elapsed));
        if (heroAction == SpriteAction.Death)
        {
        ...
        string mechanic = timeline.Events.TakeWhile(item => item.AtMilliseconds <= elapsed)
            .LastOrDefault(item => item.NodeIndex == current.NodeIndex && item.Kind is SceneEventKind.WaveStarted or SceneEventKind.MechanicChoice)?.Detail ?? string.Empty;
        DrawString(ThemeDB.FallbackFont, bounds.Position + new Vector2(16, 59),
    ...
    private void DrawSpatialAlly(Rect2 field, AllyFrame ally, SpatialFrame current, SpatialFrame next,
        long elapsed, bool hero)
    {
        AllyFrame? future = next.Allies?.FirstOrDefault(item => item.EntityId == ally.EntityId);
        float progress = next.AtMilliseconds <= current.AtMilliseconds ? 0 :
            Math.Clamp((elapsed - current.AtMilliseconds) / (float)(next.AtMilliseconds - current.AtMilliseconds), 0, 1);
        Vector2 position = MapPoint(field, Lerp(ally.Position, future?.Position ?? ally.Position, progress));
        ...
        bool hero, Vector2 position)
    {
        ...
        SceneEvent? action = _recentEvents.LastOrDefault(item => EventSource(item, ally.EntityId) &&
            IsHeroAction(item.Kind));
        SceneEvent? hit = RecentUnitHit(ally.EntityId, elapsed);
        Facing facing = FacingBetween(ally.Position, nextAlly?.Position ?? ally.Position,
            action?.EffectPosition ?? current.HeroPosition);
        SpriteAction unitAction = ResolveUnitAction(ally, nextAlly, action, hit, elapsed);
        SceneEvent? timedAction = unitAction == SpriteAction.Hit ? hit : action;
        long actionAge = unitAction is SpriteAction.Idle or SpriteAction.Move ? elapsed :
    ...
    {
        if (ally.Life <= 0) return SpriteAction.Death;
        ...
        return action is not null && atMilliseconds - action.AtMilliseconds < 500
            ? ActionForEvent(action) : SpriteAction.Idle;
    }


    private void DrawSpatialEnemy(Vector2 position, EnemyFrame enemy, float radius, bool targeted,
        Facing facing, SpriteAction action, long actionAge)
    {
        Texture2D? animation = enemy.Boss ? _bossAnimationAtlas : _enemyAnimationAtlas;
        if (animation is not null)
        {
            int rig = enemy.Boss ? ArtContract.BossRig(enemy.EnemyStableId) : ArtContract.EnemyRig(enemy.EnemyStableId);
        ...
        {
            UnitRole.Melee => new Color("91434b"),
            UnitRole.Ranged => new Color("7b5b45"),
            UnitRole.Caster => new Color("665194"),
            UnitRole.Charger => new Color("a16438"),
            UnitRole.Summoner => new Color("48705d"),
            _ => new Color("b03345"),
        };
        DrawCircle(position, radius, color);
        if (enemy.Role is UnitRole.Ranged or UnitRole.Caster)
        {
    ...
    private void DrawSpatialSkills(
        ...
        IEnumerable<SceneEvent> recent,
        long elapsed)
    {
        foreach (SceneEvent item in recent)
        {
            SkillVisualDescriptor? visual = VisualForEvent(item);
            float age = SceneEffectPlayback.Progress(item, elapsed, visual?.LifetimeMilliseconds ?? 900);
            string[] ids = item.Detail.Split('|');
            Vector2 target = item.EffectPosition is { } fixedTarget ? MapPoint(field, fixedTarget) :
                ids.Length > 1 && positions.TryGetValue(ids[1], out Vector2 live)
                ? live
                : MapPoint(field, new Point(item.Position.X * 1_000, item.Position.Y * 1_000));
            Vector2 source = item.SourcePosition is { } fixedSource ? MapPoint(field, fixedSource) :
                ids.Length > 0 && positions.TryGetValue(ids[0], out Vector2 origin) ? origin : actor;
            bool ground = item.Detail.Contains("持续危险地面", StringComparison.Ordinal);
            bool warning = SceneEffectPlayback.IsDanger(item);
            if ((ground || warning) && item.EffectPosition is { } effect && item.Presentation is { } geometry)
            {
                target = MapPoint(field, effect);
                DrawSpatialArea(field, effect, geometry.RadiusRaw, new(0, 0), false,
                    new Color(ground ? "ab4c2230" : "f0804020"), new Color(ground ? "dc772aaa" : "ffb050dd"));
                continue;
            }
            if (visual is not null && age < 1f)
            {
                if (visual.Shape == SkillShape.Chain && item.EffectPosition is { } chainTarget && item.SourcePosition is { } chainSource)
                {
                    Vector2 from = MapPoint(field, chainSource);
                    Vector2 to = MapPoint(field, chainTarget);
                    DrawLine(from, to, DamageColor(item.Detail, .78f * (1 - age)), 3.5f, true);
                    DrawLine(from.Lerp(to, .5f), to, DamageColor(item.Detail, .38f * (1 - age)), 8f, true);
                }
                if (visual.Shape == SkillShape.Self && item.Detail.Contains("召唤", StringComparison.Ordinal))
                {
                    DrawCircle(target, 18 + age * 18, new Color(0.28f, 0.8f, 0.72f, .14f * (1 - age)));
                    DrawArc(target, 18 + age * 18, 0, MathF.Tau, 24,
                        new Color(0.42f, 1f, .82f, .8f * (1 - age)), 2f);
                }
                    ...
                    SkillShape.GroundArea or SkillShape.MovementCircle)
                {
                    ...
                    Point centerPoint = visual.Shape == SkillShape.Cone ? item.SourcePosition ?? new(0, 0) :
                        item.EffectPosition ?? new(0, 0);
                    Point direction = new((item.EffectPosition?.XRaw ?? 0) - centerPoint.XRaw,
                        (item.EffectPosition?.YRaw ?? 0) - centerPoint.YRaw);
                    DrawSpatialArea(field, centerPoint, effectRange, direction, visual.Shape == SkillShape.Cone,
                        rangeColor, DamageColor(item.Detail, .7f * (1 - age)));
                }
                ...
                Vector2 center = target + variation;
                if (item.Presentation is { Shape: "projectile", Trajectory.Count: >= 2 } path)
                {
                    source = MapPoint(field, path.Trajectory[0]);
                    target = MapPoint(field, path.Trajectory[^1]);
                    center = MapPoint(field, SceneEffectPlayback.TrajectoryPosition(path, age));
                }
                DrawPresentationVfx(visual.AtlasCell, center, size, visual.UsesSourceToTarget ? source.AngleToPoint(target) : 0);
                if (visual.Signature && item.Kind != SceneEventKind.ProjectileMoved && age > .28f)
                {
                    float secondAge = Math.Clamp((age - .28f) / .58f, 0, 1);
                    DrawPresentationVfx((visual.AtlasCell + 1 + visual.VariationSeed) % 15, target,
    ...
    private void DrawSpatialArea(Rect2 field, Point center, int radius, Point direction, bool cone,
        Color fill, Color outline)
    {
        float heading = MathF.Atan2(direction.YRaw, direction.XRaw);
        float start = cone ? heading - MathF.PI / 3 : 0;
        float sweep = cone ? 2 * MathF.PI / 3 : MathF.Tau;
        var points = new List<Vector2>();
        if (cone) points.Add(MapPoint(field, center));
        for (int index = 0; index <= 48; index++)
        {
            float angle = start + sweep * index / 48;
            points.Add(MapPoint(field, new Point(center.XRaw + (int)(MathF.Cos(angle) * radius),
                center.YRaw + (int)(MathF.Sin(angle) * radius))));
        }
        Vector2[] clip = [field.Position, new(field.End.X, field.Position.Y), field.End,
            new(field.Position.X, field.End.Y)];
        foreach (Vector2[] polygon in Geometry2D.IntersectPolygons(points.ToArray(), clip))
        {
            if (polygon.Length < 3) continue;
            DrawColoredPolygon(polygon, fill);
            DrawPolyline(polygon.Append(polygon[0]).ToArray(), outline, 1.5f, true);
        }
    }
    ...
    private void DrawSpatialNumbers(
        ...
        long elapsed)
    {
        if (VisualPreferences.DamageNumbers == DamageNumberMode.Off) return;
        SceneEvent[] candidates = recent.Where(item => item.Value > 0 && item.Kind is
                SceneEventKind.HeavyStrike or SceneEventKind.EarthCleave or SceneEventKind.SpiritBlade or
                SceneEventKind.Chain or SceneEventKind.SeismicCharge or SceneEventKind.BloodTideSpin or
                SceneEventKind.AshJavelin or SceneEventKind.EmberNova or SceneEventKind.StormBrand or
                SceneEventKind.EnemyAttack or SceneEventKind.Bleed or SceneEventKind.SkillEffect or
                SceneEventKind.Ascendancy or SceneEventKind.Ailment).ToArray();
        List<(SceneEvent Event, int Value)> entries;
        if (VisualPreferences.DamageNumbers == DamageNumberMode.Full)
        {
        ...
        int lane = 0;
        foreach ((SceneEvent item, int value) in entries.Take(VisualPreferences.EffectLimit(Size.X) * 2))
        {
            float age = Math.Clamp((elapsed - item.AtMilliseconds) / 900f, 0, 1);
            string[] ids = item.Detail.Split('|');
            bool heroDamage = item.Kind == SceneEventKind.EnemyAttack ||
                              item.Kind == SceneEventKind.Bleed && ids.Length > 1 && ids[1] == "hero";
    ...
    private static Vector2 MapPoint(Rect2 field, Point point) => new(
    ...
    private static Point Lerp(Point from, Point to, float weight) => new(
        (int)MathF.Round(Mathf.Lerp(from.XRaw, to.XRaw, weight)),
        (int)MathF.Round(Mathf.Lerp(from.YRaw, to.YRaw, weight)));

    private void DrawArtSprite(Texture2D? atlas, int rig, Facing facing, SpriteAction action,
        ...
        bool loop = true, Color? tint = null)
    {
        ...
        int column = ArtContract.AnimationColumn(action, Math.Max(0, elapsed), loop);
        int row = ArtContract.AnimationRow(rig, facing, rigCount);
        Rect2 source = ArtAtlas.AnimationCell(column, row, cellWidth, cellHeight);
    ...
    private static Facing FacingBetween(Point from, Point to, Point fallback)
    {
        ...
        if (Math.Abs(x) >= Math.Abs(y)) return x < 0 ? Facing.Left : Facing.Right;
        return y < 0 ? Facing.Up : Facing.Down;
    }

    private static SpriteAction ResolveHeroAction(SpatialFrame current, SpatialFrame next,
        IReadOnlyList<SceneEvent> recent)
    {
        if (current.HeroLife <= 0) return SpriteAction.Death;
        if (recent.Any(item => EventTargets(item, "hero") && item.Kind is SceneEventKind.EnemyAttack or SceneEventKind.Bleed))
            return SpriteAction.Hit;
        SceneEvent? action = recent.LastOrDefault(item => EventSource(item, "hero") && IsHeroAction(item.Kind));
        if (action is not null) return ActionForEvent(action);
        return current.HeroPosition != next.HeroPosition ? SpriteAction.Move : SpriteAction.Idle;
    }

    private static SpriteAction ResolveEnemyAction(EnemyFrame current, EnemyFrame next,
        IReadOnlyList<SceneEvent> recent)
    {
        if (current.Life <= 0) return SpriteAction.Death;
        if (recent.Any(item => EventTargets(item, current.EntityId) && IsDamageEvent(item.Kind))) return SpriteAction.Hit;
        if (recent.Any(item => item.Kind == SceneEventKind.EnemyAttack && EventSource(item, current.EntityId)))
            return current.Role is UnitRole.Caster or UnitRole.Summoner ? SpriteAction.Cast : SpriteAction.Attack;
        return current.Position != next.Position ? SpriteAction.Move : SpriteAction.Idle;
    }

    private static SpriteAction ActionForEvent(SceneEvent item)
    {
        SkillVisualDescriptor? visual = VisualForEvent(item);
        return visual is not null ? SkillAnimation.ForSkill(visual.SkillId) :
            item.Kind is SceneEventKind.WarCry or SceneEventKind.Banner or SceneEventKind.Guard or SceneEventKind.Flask
                ? SpriteAction.Cast : SpriteAction.Attack;
    }

    private static bool IsHeroAction(SceneEventKind kind) => kind is
        SceneEventKind.WarCry or SceneEventKind.HeavyStrike or SceneEventKind.EarthCleave or
        SceneEventKind.SpiritBlade or SceneEventKind.SeismicCharge or SceneEventKind.BloodTideSpin or
        SceneEventKind.Banner or SceneEventKind.AshJavelin or SceneEventKind.EmberNova or
        SceneEventKind.StormBrand or SceneEventKind.SkillEffect or SceneEventKind.Guard or SceneEventKind.Flask;

    private static bool IsDamageEvent(SceneEventKind kind) => kind is
        SceneEventKind.HeavyStrike or SceneEventKind.EarthCleave or SceneEventKind.SpiritBlade or
        SceneEventKind.Chain or SceneEventKind.SeismicCharge or SceneEventKind.BloodTideSpin or
        SceneEventKind.AshJavelin or SceneEventKind.EmberNova or SceneEventKind.StormBrand or
        SceneEventKind.EnemyAttack or SceneEventKind.Bleed or SceneEventKind.SkillEffect;

    private static bool HasRecentEvent(IEnumerable<SceneEvent> events, SceneEventKind kind,
    ...
    private static long EventAge(IEnumerable<SceneEvent> events, long elapsed, SceneEventKind? kind, string entityId)
    {
        SceneEvent? item = events.LastOrDefault(candidate =>
    ...
    private static bool EventSource(SceneEvent item, string entityId)
    {
        int separator = item.Detail.IndexOf('|');
        return separator > 0 && item.Detail.AsSpan(0, separator).SequenceEqual(entityId);
    }

    private static bool EventTargets(SceneEvent item, string entityId)
    {
    ...
    private void DrawEnemyRoleMarker(Vector2 position, EnemyFrame enemy)
    {
        int variant = ArtContract.EnemyVariant(enemy.EnemyStableId);
    ...
    {
        if (_presentationVfxAtlas is null) return;
        Rect2 source = new((index % 4) * 64, (index / 4) * 64, 64, 64);
        DrawSetTransform(center, rotation, Vector2.One);
        DrawTextureRectRegion(_presentationVfxAtlas, new Rect2(new Vector2(-size / 2, -size / 2), new Vector2(size, size)), source);
        DrawSetTransform(Vector2.Zero, 0, Vector2.One);
    }

    private void DrawHitAccent(SceneEvent item, Vector2 center, float age)
    {
        if (!SceneEffectPlayback.IsCriticalHit(item)) return;
    ...
    private void DrawCombatFeedback(Rect2 field, IReadOnlyDictionary<string, Vector2> positions,
        IEnumerable<SceneEvent> recent, long elapsed)
    {
        int lane = 0;
        foreach (SceneEvent item in recent.Where(NeedsFeedback).OrderByDescending(item => item.AtMilliseconds)
                     .Take(VisualPreferences.EffectLimit(Size.X)))
        {
            string[] ids = item.Detail.Split('|');
            string targetId = ids.Length > 1 ? ids[1] : "hero";
            Vector2 target = positions.TryGetValue(targetId, out Vector2 live)
                ? live : MapPoint(field, new Point(item.Position.X * 1_000, item.Position.Y * 1_000));
            float age = Math.Clamp((elapsed - item.AtMilliseconds) / 850f, 0, 1);
            string text = FeedbackText(item);
            if (text.Length == 0) continue;
            Color color = item.Kind switch
            {
                SceneEventKind.Block or SceneEventKind.Guard => new Color(.4f, .85f, 1, 1 - age),
                SceneEventKind.SkillFailed => new Color(1, .45f, .35f, 1 - age),
                SceneEventKind.Ailment => new Color(.65f, .9f, .36f, 1 - age),
                SceneEventKind.EnemyDefeated => new Color(1, .78f, .3f, 1 - age),
    ...
    private void DrawRewardStrip(Rect2 bounds, bool hero)
    {
        ...
        TeamExpeditionState team = hero ? _session.World.Hero : _session.World.Mercenaries;
    ...
    private void DrawEnemyStatusIcons(Vector2 position, EnemyFrame enemy)
    {
        ...
        if (enemy.DamageOverTimeAilment != Ailment.None)
    ...
    private void DrawVirtueViceIcons(Vector2 actor, IReadOnlyDictionary<VirtueViceKind, int>? layers)
    {
        if (layers is null || layers.Count == 0) return;
        int index = 0;
        foreach ((VirtueViceKind kind, int count) in layers.OrderBy(item => item.Key))
        {
            Color color = kind switch
            {
                VirtueViceKind.Mercy => new Color("70c98b"),
                VirtueViceKind.Temperance => new Color("7bb6db"),
                VirtueViceKind.Humility => new Color("d1c29a"),
                VirtueViceKind.Rage => new Color("db4a3f"),
                VirtueViceKind.Sloth => new Color("8971c7"),
                _ => new Color("d4a14e"),
            };
    ...
    {
        Ailment.Ignite => new Color("f06a38"),
        Ailment.Chill or Ailment.Freeze => new Color("68cde5"),
        Ailment.Shock or Ailment.Paralysis => new Color("f0d64c"),
        Ailment.Erosion or Ailment.Wither => new Color("a56bd6"),
        _ => new Color("70c96d"),
    };

    private static SkillVisualDescriptor? VisualForEvent(SceneEvent item)
    {
        string skillId = ReadDetailValue(item.Detail, "skill:") ?? item.Kind switch
        {
            SceneEventKind.HeavyStrike => SkillIds.HeavyStrike,
            SceneEventKind.EarthCleave => SkillIds.EarthCleave,
            SceneEventKind.Aftershock => SkillIds.SeismicCharge,
            SceneEventKind.SpiritBlade or SceneEventKind.Chain => SkillIds.SpiritBlade,
            SceneEventKind.SeismicCharge => SkillIds.SeismicCharge,
            SceneEventKind.BloodTideSpin => SkillIds.BloodTideSpin,
            SceneEventKind.AshJavelin => SkillIds.AshJavelin,
            SceneEventKind.EmberNova => SkillIds.EmberNova,
            SceneEventKind.StormBrand => SkillIds.StormBrand,
            SceneEventKind.WarCry => SkillIds.WarCry,
            SceneEventKind.Banner => SkillIds.IronOathBanner,
            _ => string.Empty,
        };
        return skillId.Length > 0 && VisualCatalog.TryForSkill(skillId, out SkillVisualDescriptor? result)
            ? result : null;
    }
    ...
    private static bool NeedsFeedback(SceneEvent item)
    {
        if (item.Kind is SceneEventKind.Block or SceneEventKind.Guard or SceneEventKind.SkillFailed or
            SceneEventKind.Ailment or SceneEventKind.EnemyDefeated or SceneEventKind.BossPhase) return true;
    ...
    private static string FeedbackText(SceneEvent item)
    {
        ...
        {
            SceneEventKind.Block => "格挡",
            SceneEventKind.Guard => "减伤",
            SceneEventKind.SkillFailed => "资源不足",
            SceneEventKind.EnemyDefeated => "击破",
            SceneEventKind.BossPhase => item.Detail.Contains("enraged", StringComparison.Ordinal) ? "狂暴" : "阶段变化",
            SceneEventKind.Ailment => AilmentText(ReadDetailValue(item.Detail, "ailment:") ?? ReadDetailValue(item.Detail, "dot:")),
            _ => string.Empty,
        };
    }
    ...
    {
        if (item.Kind == SceneEventKind.EnemyAttack) return "hero";
    ...
    private Vector2 ScreenShakeOffset()
    {
        ...
        IReadOnlyList<SceneEvent> events = observed.Timeline.Events;
        for (int index = events.Count - 1; index >= 0; index--)
        {
            SceneEvent item = events[index];
            long age = observed.ElapsedMilliseconds - item.AtMilliseconds;
            if (age > 180) break;
            if (age < 0 || item.Kind is not (SceneEventKind.HeavyStrike or SceneEventKind.BossPhase or
                SceneEventKind.EnemyDefeated or SceneEventKind.Ascendancy)) continue;
    ...
    private static int FindFrameIndex(IReadOnlyList<SpatialFrame> frames, long elapsed)
    {
        int low = 0;
    ...
    private static void CollectRecentEvents(IReadOnlyList<SceneEvent> events, long elapsed, List<SceneEvent> destination)
    {
        destination.Clear();
    ...
    private ObservedScene? Observe()
    {
        ...
        if (_mode == ViewMode.Active && !_session.Campaign.Completed && _session.Campaign.ActiveTimeline is not null)
        {
    ...
    private static ObservedScene? ObserveExpedition(TeamExpeditionState team, bool hero)
    {
    ...
    private void DrawCompactSkillEffects(Vector2 actor, Vector2 enemy, IEnumerable<SceneEvent> recent)
    {
        foreach (SceneEvent item in recent)
        {
            SkillVisualDescriptor? visual = VisualForEvent(item);
            ...
            Vector2 center = visual.UsesSourceToTarget
                ...
                : item.Kind is SceneEventKind.WarCry or SceneEventKind.Banner ? actor : enemy;
    ...
    private void DrawFloatingNumbers(Vector2 enemy, Vector2 actor, IEnumerable<SceneEvent> recent, long elapsed)
    {
        if (VisualPreferences.DamageNumbers == DamageNumberMode.Off) return;
        int lane = 0;
        foreach (SceneEvent item in recent.Where(item => item.Value > 0 && item.Kind is
                     SceneEventKind.HeavyStrike or SceneEventKind.Aftershock or
                     SceneEventKind.EnemyAttack or SceneEventKind.Bleed))
        {
            float age = Math.Clamp((elapsed - item.AtMilliseconds) / 1_100f, 0, 1);
            bool hurtsHero = item.Kind == SceneEventKind.EnemyAttack ||
                             item.Kind == SceneEventKind.Bleed && item.Detail == "hero";
    ...
    private sealed record ObservedScene(SceneTimeline Timeline, long ElapsedMilliseconds, string Title, bool Hero);
    ...
    {
        for (int index = 0; index < MapCatalog.Areas.Count; index++)
            if (stableId.Contains($":{MapCatalog.Areas[index].StableId}:", StringComparison.Ordinal)) return index;
        return StableVisualIndex(stableId, MapCatalog.Areas.Count);
    }
