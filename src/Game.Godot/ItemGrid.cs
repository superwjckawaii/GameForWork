...
{
    private readonly List<ItemCell> _cells = [];
    ...
    {
        _selectedIndex = -1;
        foreach (ItemCell cell in _cells) cell.SetPressedNoSignal(false);
    }
    ...
    private void ApplyCells()
    {
        for (int index = 0; index < _cells.Count; index++)
        {
            ...
            cell.Text = occupied && cell.Icon is null ? UiText.ItemGlyph(item!.Base.Category) :
            ...
            cell.TooltipText = occupied
                ? UiText.ItemTooltip(item!) + (extra.Length == 0 ? string.Empty : $"\n\n{extra}")
                : $"空格 {index + 1}";
            cell.Disabled = _filtered && !occupied;
            cell.SetPressedNoSignal(index == _selectedIndex);
            Color color = occupied ? UiText.RarityColor(item!.Rarity) : new Color("71695e");
