...
public partial class ExpeditionPanel : VBoxContainer
{
    ...
    private MapCraftWindow? _mapCraftWindow;
    ...
    public void Initialize(Func<GameSession> session, Action<string> changed)
    {
        ...
        _session = session;
        _changed = changed;
        _mapFilterWindow = new MapFilterWindow();
        _mapFilterWindow.Initialize();
        AddChild(_mapFilterWindow);
        _mapCraftWindow = new MapCraftWindow();
        ...
        batch.Pressed += () =>
        {
            MapBatchResult result = _session!().BatchCraftMaps(_session!().World.MapCraftRule, _session!().World.MapCraftFilter);
    ...
    public void RefreshState()
    {
        ...
        {
            MapItem source = session.World.MapInventory[index];
            MapItem formal = source.EnsureFormal(session.Seed ^ (ulong)index);
        ...
        var mapGroups = session.World.MapInventory
            ...
            .ToArray();
        _resources.Text =
            $"地图 {session.World.MapInventory.Count}　深渊监守者碎片 {session.World.Expedition.AbyssWardenFragments}/{ExpeditionDirector.FragmentsPerTicket}　" +
        ...
        if (mapSignature != _mapSignature)
        {
            ...
            foreach (var group in mapGroups)
            {
                ...
                var representative = entries
                    ...
                    .First();
                MapItem map = representative.Map;
                MapArea area = ResolveArea(map);
                int rare = entries.Count(entry => entry.Map.Rarity == MapRarity.Rare);
                int magic = entries.Count(entry => entry.Map.Rarity == MapRarity.Magic);
        ...
        if (_reports.Visible && reportSignature != _reportSignature)
        {
            ...
            Clear(_reports);
            foreach (CombatReport report in session.World.Expedition.Reports.Reverse())
            {
        ...
        {
            TeamExpeditionState team = kind == ExpeditionTeamKind.Hero ? session.World.Hero : session.World.Mercenaries;
            TeamDispatchSnapshot? dispatch = session.World.Expedition.Get(kind);
            if (dispatch is not null && dispatchSignature != _dispatchSignature)
            {
                if (controls.Boss is { } activeBoss && ExpeditionDirector.IsBossTarget(dispatch.Target))
                {
                    ...
                    activeBoss.Count.Visible = dispatch.Mode == DispatchMode.Once;
                    if (dispatch.Mode == DispatchMode.Once && dispatch.RemainingRuns > 0)
                        activeBoss.Count.Value = dispatch.RemainingRuns;
                }
                else
                {
                    controls.Mode.Select(controls.Mode.GetItemIndex((int)dispatch.Mode));
                    controls.RunCount.Visible = dispatch.Mode == DispatchMode.Once;
                    if (dispatch.Mode == DispatchMode.Once && dispatch.RemainingRuns > 0)
            ...
            if (controls.Boss is { } boss)
            {
                ...
                SetBossItemText(boss.Target, ExpeditionTarget.AbyssWardenPractice, "深渊监守者练习（免费）");
                SetBossItemText(boss.Target, ExpeditionTarget.AshenCitadel,
                    $"灰烬天垒（门票 ×{session.Endgame.CitadelTickets}）");
                SetBossItemText(boss.Target, ExpeditionTarget.AshenCitadelPractice, "灰烬天垒练习（免费）");
                SetBossItemText(boss.Target, ExpeditionTarget.FinalBreakthrough,
                    session.Endgame.FinalBreakthroughCompleted ? "百级门扉（已完成）" :
                    session.World.Hero.Progression.Level >= 100 ? "百级门扉（可挑战）" : "百级门扉（需要 Lv100）");
                ExpeditionTarget selected = (ExpeditionTarget)boss.Target.GetItemId(boss.Target.Selected);
                BossChallengeAvailability availability = session.GetBossChallengeAvailability(selected);
    ...
    private Control BuildTeamCard(ExpeditionTeamKind kind, string title)
    {
        ...
        content.AddChild(BuildMapFilterButton(
            ...
            "通用地图筛选"));
        var gameplay = new GameplayPanel();
        ...
        mode.AddItem("执行指定次数", (int)DispatchMode.Once);
        mode.AddItem("重复执行", (int)DispatchMode.Repeat);
        mode.AddItem("最高阶推进", (int)DispatchMode.HighestAvailable);
        ...
        mode.ItemSelected += _ => runCount.Visible = (DispatchMode)mode.GetItemId(mode.Selected) == DispatchMode.Once;
        var start = new Button { Text = "开始远征" };
        void StartMapExpedition()
        {
            DispatchMode selectedMode = (DispatchMode)mode.GetItemId(mode.Selected);
            TeamExpeditionState team = kind == ExpeditionTeamKind.Hero ? _session!().World.Hero : _session!().World.Mercenaries;
            ExpeditionPolicy current = team.PendingPolicy ?? team.Policy;
            MapFilter filter = current.MapFilter ?? MapFilter.All;
            ...
            _session!().SetExpeditionPolicy(kind, current with
            {
                ...
                MapFilter = filter,
                MapOrder = MapOrder.Recommended,
                NoMatchBehavior = NoMatchBehavior.Stop,
                UseRareFragments = false,
                Gameplay = GameplayPanel.AutomaticDifficulty(),
            });
            _session!().AssignExpedition(kind, ExpeditionTarget.HighestTierMaps, selectedMode, (int)runCount.Value);
            _mapSignature = string.Empty;
            _changed?.Invoke(selectedMode == DispatchMode.Once
        ...
        abandon.Pressed += () =>
        {
            TeamExpeditionState team = kind == ExpeditionTeamKind.Hero
                ? _session!().World.Hero
    ...
    private BossControls BuildBossControls(Control parent)
    {
        ...
        target.AddItem("深渊监守者", (int)ExpeditionTarget.AbyssWarden);
        target.AddItem("深渊监守者练习", (int)ExpeditionTarget.AbyssWardenPractice);
        target.AddItem("灰烬天垒", (int)ExpeditionTarget.AshenCitadel);
        target.AddItem("灰烬天垒练习", (int)ExpeditionTarget.AshenCitadelPractice);
        target.AddItem("百级门扉", (int)ExpeditionTarget.FinalBreakthrough);
        row.AddChild(target);
        var mode = new OptionButton();
        mode.AddItem("执行指定次数", (int)DispatchMode.Once);
        mode.AddItem("重复执行", (int)DispatchMode.Repeat);
        ...
        row.AddChild(count);
        mode.ItemSelected += _ => count.Visible = (DispatchMode)mode.GetItemId(mode.Selected) == DispatchMode.Once;
        target.ItemSelected += _ =>
        {
            bool onceOnly = (ExpeditionTarget)target.GetItemId(target.Selected) == ExpeditionTarget.FinalBreakthrough;
            mode.Disabled = onceOnly;
            if (onceOnly) mode.Select(mode.GetItemIndex((int)DispatchMode.Once));
            count.Visible = onceOnly || (DispatchMode)mode.GetItemId(mode.Selected) == DispatchMode.Once;
    ...
    private static void SetBossItemText(OptionButton target, ExpeditionTarget id, string text)
    {
    ...
    private static bool HasBossChallenge(GameSession session)
    {
        bool bossMap = session.World.Hero.ActiveMap is { } active &&
            (ExpeditionDirector.IsBoss(active) || ExpeditionDirector.IsPractice(active));
        bossMap |= session.World.Hero.Queue.Maps.Any(map =>
            ExpeditionDirector.IsBoss(map) || ExpeditionDirector.IsPractice(map));
        TeamDispatchSnapshot? dispatch = session.World.Expedition.Get(ExpeditionTeamKind.Hero);
        return bossMap || dispatch is not null && ExpeditionDirector.IsBossTarget(dispatch.Target) && dispatch.Enabled;
    }

    private static bool HasMapExpedition(GameSession session)
    {
        ...
        normalMap |= session.World.Hero.Queue.Maps.Any(map =>
            !ExpeditionDirector.IsBoss(map) && !ExpeditionDirector.IsPractice(map));
        TeamDispatchSnapshot? dispatch = session.World.Expedition.Get(ExpeditionTeamKind.Hero);
        return normalMap || dispatch is not null && !ExpeditionDirector.IsBossTarget(dispatch.Target) && dispatch.Enabled;
    }

    private static string BossName(ExpeditionTarget target) => target switch
    {
        ExpeditionTarget.AbyssWarden => "深渊监守者",
        ExpeditionTarget.AbyssWardenPractice => "深渊监守者练习",
        ExpeditionTarget.AshenCitadel => "灰烬天垒",
        ExpeditionTarget.AshenCitadelPractice => "灰烬天垒练习",
        ExpeditionTarget.FinalBreakthrough => "百级门扉",
        _ => "未知 Boss",
    };
    ...
    {
        GameSession session = _session!();
        MapFilter current = FilterFor(session, scope);
    ...
    private void RefreshFilterCounts(GameSession session)
    {
    ...
    private static MapFilter FilterFor(GameSession session, MapFilterScope scope) => scope switch
    {
        MapFilterScope.Hero => (session.World.Hero.PendingPolicy ?? session.World.Hero.Policy).MapFilter ?? MapFilter.All,
        MapFilterScope.Mercenaries => (session.World.Mercenaries.PendingPolicy ?? session.World.Mercenaries.Policy).MapFilter ?? MapFilter.All,
        _ => session.World.MapCraftFilter,
    };

    private static int MatchingMapCount(GameSession session, MapFilter filter) =>
    ...
    private static string ModeName(DispatchMode mode) => mode switch
    {
        DispatchMode.Repeat => "重复执行",
        DispatchMode.HighestAvailable => "最高阶推进",
        _ => "执行指定次数",
    };

    private static string TeamStatus(TeamExpeditionState team, TeamDispatchSnapshot? dispatch)
    {
        if (team.ActiveMap is not null)
        {
            string target = EndgameState.IsCitadel(team.ActiveMap) ? "灰烬天垒" :
                ExpeditionDirector.IsPractice(team.ActiveMap) ? "Boss 练习" :
                ExpeditionDirector.IsBoss(team.ActiveMap) ? "深渊监守者" : $"T{team.ActiveMap.Tier} · Lv{team.ActiveMap.MonsterLevel} 地图";
        ...
        {
            MapItem map = team.Queue.Maps[0];
            return $"准备中：{TargetName(dispatch?.Target ?? ExpeditionTarget.SafeMaps)}{ModeStatus(dispatch)} · T{map.Tier} · Lv{map.MonsterLevel}";
        }
    ...
    private static string ModeStatus(TeamDispatchSnapshot? dispatch) => dispatch?.Mode switch
    {
        DispatchMode.Repeat => " · 重复执行",
        DispatchMode.Once when dispatch.RemainingRuns > 1 => $" · 剩余 {dispatch.RemainingRuns} 次",
        _ => string.Empty,
    };

    private static string TeamSignature(TeamExpeditionState team) =>
    ...
    private static string TargetName(ExpeditionTarget target) => target switch
    {
        ExpeditionTarget.SafeMaps => "安全探索",
        ExpeditionTarget.AbyssMaps => "裂渊追猎",
        ExpeditionTarget.LifeGardenMaps => "命能花园",
        ExpeditionTarget.WarfrontMaps => "亡旗战阵",
        ExpeditionTarget.HighestTierMaps => "最高阶推进",
        ExpeditionTarget.AbyssWarden => "深渊监守者",
        ExpeditionTarget.AbyssWardenPractice => "Boss 练习",
        ExpeditionTarget.AshenCitadel => "灰烬天垒",
        ExpeditionTarget.AshenCitadelPractice => "灰烬天垒练习",
        ExpeditionTarget.FinalBreakthrough => "百级门扉",
        _ => "未知目标",
    ...
    private static MapArea ResolveArea(MapItem map) =>
        MapCatalog.TryGet(map.AreaId, out MapArea area)
            ? area
            : new MapArea(map.AreaId, "未登记路印", "未知区域", "未知敌群", "未知首领");
