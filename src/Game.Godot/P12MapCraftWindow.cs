...
public partial class P12MapCraftWindow : P30IndependentWindow
{
    ...
    private Action<P12MapBatchRule>? _save;
    ...
    public void Initialize()
    {
        ...
        _rarity = new OptionButton(); _rarity.AddItem("目标：魔法", (int)P12MapRarity.Magic); _rarity.AddItem("目标：稀有", (int)P12MapRarity.Rare); form.AddChild(_rarity);
        _excludedMenu = new MenuButton { Text = "排除词缀（无）", TooltipText = "可多选；再次点击取消。" };
        PopupMenu popup = _excludedMenu.GetPopup();
        foreach (P26MapAffixDefinition definition in P26MapAffixCatalog.All) popup.AddCheckItem(definition.DisplayName, (int)definition.Kind);
        popup.IdPressed += id =>
        {
        ...
        _failure = new OptionButton(); _failure.AddItem("保留地图", (int)P12BatchFailureBehavior.Keep); _failure.AddItem("出售地图", (int)P12BatchFailureBehavior.Sell);
    ...
    public void Open(P12MapBatchRule rule, Action<P12MapBatchRule> save)
    {
        _save = save;
    ...
    private P12MapBatchRule Read() => new(
        (P12MapRarity)_rarity!.GetItemId(_rarity.Selected),
        (int)_quality!.Value,
        _excluded.Select(id => (P12MapAffixKind)id).ToArray(),
        _fill!.ButtonPressed,
        _corrupt!.ButtonPressed,
        (P12BatchFailureBehavior)_failure!.GetItemId(_failure.Selected));
}
