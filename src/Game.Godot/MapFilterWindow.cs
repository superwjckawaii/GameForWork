...
public partial class MapFilterWindow : IndependentWindow
{
    ...
    private Action<MapFilter>? _save;
    ...
    public void Initialize()
    {
        ...
        _rarity.AddItem("普通", (int)MapRarity.Basic);
        _rarity.AddItem("魔法", (int)MapRarity.Magic);
        _rarity.AddItem("稀有", (int)MapRarity.Rare);
        ...
        _areaMenu = AddMultiSelect(advanced, "区域（全部）",
            MapCatalog.Areas.Select((area, index) => (area.DisplayName, index)), _areas);
        _requiredMenu = AddMultiSelect(advanced, "必含词缀（无）",
            MapAffixCatalog.All.Select(definition => (definition.DisplayName, (int)definition.Kind)), _required);
        _excludedMenu = AddMultiSelect(advanced, "排除词缀（无）",
            MapAffixCatalog.All.Select(definition => (definition.DisplayName, (int)definition.Kind)), _excluded);

        var actions = new HBoxContainer { Alignment = BoxContainer.AlignmentMode.End };
        var reset = new Button { Text = "恢复全部地图" };
        reset.Pressed += () => LoadFilter(MapFilter.All);
        var save = new Button { Text = "保存筛选" };
        save.Pressed += () =>
        {
            MapFilter filter = ReadFilter().Validate();
            _save?.Invoke(filter);
            Hide();
        };
    ...
    public void Open(string scope, MapFilter filter, Action<MapFilter> save)
    {
        _scope!.Text = scope;
        _save = save;
        LoadFilter(filter);
    ...
    private MapFilter ReadFilter()
    {
        ...
        return new MapFilter(
            ...
            12_000,
            _areas.Select(index => MapCatalog.Areas[index].StableId).ToArray(),
            rarityId < 0 ? [] : [(MapRarity)rarityId],
            corruption != 2,
            corruption != 1,
            (int)_minimumQuality!.Value,
            _required.Select(id => (MapAffixKind)id).ToArray(),
            _excluded.Select(id => (MapAffixKind)id).ToArray());
    }

    private void LoadFilter(MapFilter filter)
    {
        ...
        LoadSelections(_areaMenu!, _areas, filter.AreaIds?.Select(id =>
            MapCatalog.Areas.Select((area, index) => (area, index)).First(pair => pair.area.StableId == id).index) ?? []);
