...
public partial class TownPanel : VBoxContainer
{
    ...
    public void Initialize(Func<GameSession> session, Action<string> changed)
    {
        _session = session;
        _changed = changed;
        const string artActors = "res://assets/art/characters/art-actor-animation.png";
        _mercenaryAtlas = ResourceLoader.Exists(artActors) ? GD.Load<Texture2D>(artActors) : null;
        const string artBuildings = "res://assets/art/town/art-building-atlas.png";
        _buildingAtlas = ResourceLoader.Exists(artBuildings) ? GD.Load<Texture2D>(artBuildings) : null;
        var top = new HBoxContainer();
        AddChild(top);
        top.AddChild(new Label { Text = "固定城区 · 城镇方针" });
        _policy = new OptionButton();
        _policy.AddItem("开拓：施工速度 +20%", (int)TownPolicy.Expansion);
        _policy.AddItem("远征：地图产出速度 +20%", (int)TownPolicy.Expedition);
        _policy.AddItem("练兵：后备经验提高", (int)TownPolicy.Training);
        _policy.ItemSelected += index => { session().SetTownPolicy((TownPolicy)_policy.GetItemId((int)index)); changed("城镇方针已切换。"); };
    ...
    private void RebuildBuildings(GameSession session)
    {
        foreach (Node child in _buildings!.GetChildren()) child.QueueFree();
        BuildingKind[] layout = [BuildingKind.Tavern, BuildingKind.Workshop, BuildingKind.Alchemy,
            BuildingKind.Cartography, BuildingKind.Storage, BuildingKind.Reliquary, BuildingKind.Teleporter];
        for (int layoutIndex = 0; layoutIndex < layout.Length; layoutIndex++)
        {
            BuildingKind kind = layout[layoutIndex];
            int level = session.Town.Level(kind);
            ConstructionSnapshot? job = session.Town.Construction.FirstOrDefault(item => item.Kind == kind);
            BuildingUpgradeCost? cost = TownState.NextUpgradeCost(level);
            ...
            string tooltip = $"{TownState.DisplayName(kind)} · Lv.{level}\n当前：{BuildingEffect(kind, level)}\n{BuildingTooltip(kind)}" +
            ...
            if (_buildingAtlas is not null)
            {
                ...
                {
                    Texture = ArtAtlas.Icon(_buildingAtlas, layoutIndex, 4),
            ...
            {
                Text = $"{TownState.DisplayName(kind)} · Lv.{level}",
    ...
    private void RebuildCandidates(GameSession session)
    {
        foreach (Node child in _candidates!.GetChildren()) child.QueueFree();
        foreach (MercenaryCandidate candidate in session.Town.Candidates)
        {
    ...
    private void RebuildRoster(GameSession session)
    {
        foreach (Node child in _roster!.GetChildren()) child.QueueFree();
        foreach (MercenaryMember member in session.Town.Roster)
        {
    ...
    private TextureRect MercenaryPortrait(MercenaryArchetype archetype)
    {
        AtlasTexture? texture = null;
        if (_mercenaryAtlas is not null)
        {
            int rig = (int)archetype + 1;
            Rect2 region = ArtAtlas.AnimationCell(0, rig * 4, 48, 64);
    ...
    private static string BuildingEffect(BuildingKind kind, int level) => kind switch
    {
        BuildingKind.Tavern => $"名册 {level * 2 + 4} 人", BuildingKind.Workshop => $"附魔阶级 {level}",
        BuildingKind.Alchemy => $"炼金配方阶级 {level}", BuildingKind.Cartography => $"生成最高 T{Math.Min(10, level * 2 + 2)}",
        BuildingKind.Storage => $"容量 {level switch { 1 => 100, 2 => 150, 3 => 225, _ => 325 }}",
        BuildingKind.Reliquary => $"里程碑奖励 ×{level}", BuildingKind.Teleporter => $"佣兵上阵 {level + 2} 人", _ => string.Empty,
    };
    private static string BuildingTooltip(BuildingKind kind) => kind switch
    {
        BuildingKind.Tavern => "候选刷新、锁定、招募、后备经验和名册容量。",
        BuildingKind.Workshop => "解锁更强工匠词缀与独立附魔；加工仍在角色与物品页完成。",
        BuildingKind.Alchemy => "解锁公开、确定性的金属兑换配方。",
        BuildingKind.Cartography => "定时生成路印；远征方针会提高速度。",
        BuildingKind.Storage => "扩充装备仓库并管理无限堆叠的金属仓。",
        BuildingKind.Reliquary => "记录首杀、传奇收藏和城镇里程碑。",
        _ => "提高佣兵队人数上限并缩短远征准备。",
    };
    private static string Archetype(MercenaryArchetype kind) => kind switch
    { MercenaryArchetype.Guardian => "守卫", MercenaryArchetype.Ranger => "游猎者", MercenaryArchetype.Cantor => "颂仪者", _ => "秘械师" };
    private static string Potential(MercenaryPotential kind) => kind switch
    { MercenaryPotential.Common => "普通", MercenaryPotential.Promising => "优秀", MercenaryPotential.Exceptional => "卓越", _ => "传奇" };
}
