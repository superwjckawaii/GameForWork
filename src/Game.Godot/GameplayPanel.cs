...
public partial class GameplayPanel : VBoxContainer
{
    public static GameplayPolicy AutomaticDifficulty() => new(
        Reward: RewardPreference.Balanced,
        AbyssIntensity: 5,
        AbyssStages: 4,
        AbyssFinalGuardian: true,
        Garden: GardenMode.Triple,
        Red: AltarMode.Extreme,
        Blue: AltarMode.Extreme,
        Warfront: WarfrontMode.Decisive,
        RejectedCosts: [],
        Refresh: Refresh.Always,
        GardenTag: "life");

    public void Initialize(Func<GameSession> session, ExpeditionTeamKind teamKind, Action<string> changed)
    {
        TeamExpeditionState Team() => teamKind == ExpeditionTeamKind.Hero
            ? session().World.Hero
