...
public partial class CampaignRouteView : VBoxContainer
{
    ...
    public void Initialize(Func<GameSession> session, Action<string> selected)
    {
        _session = session;
        _selected = selected;
        string[] actPaths = Enumerable.Range(1, 5)
            .Select(index => $"res://assets/art/regions/act-{index}.png")
            .ToArray();
        ...
        for (int act = 1; act <= 5; act++)
        {
            ...
            {
                Text = $"{act}. {CampaignCatalog.ActNames[act - 1]}",
                CustomMinimumSize = new Vector2(106, 0),
            });
            foreach (CampaignNodeDefinition node in CampaignCatalog.Nodes.Where(node => node.Act == act))
            {
    ...
    public void RefreshState()
    {
        ...
        CampaignState campaign = _session().Campaign;
        ...
        foreach (CampaignNodeDefinition node in CampaignCatalog.Nodes)
        {
