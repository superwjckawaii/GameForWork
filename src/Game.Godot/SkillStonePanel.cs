...
public partial class SkillStonePanel : VBoxContainer
{
    ...
    public void RefreshState()
    {
        ...
        ManagementState management = session.Management;
        ...
        {
            Color stoneColor = SkillStoneVisual.ColorFor(stone.Definition.StableId);
            Texture2D? stoneIcon = _art?.SkillIcon(stone.Definition.StableId);
            var cell = new SkillStoneCell
            {
                ...
                TooltipText = SkillTooltip.Build(stone, "技能石背包", management.HeldSkillStoneCount(stone.DefinitionId, stone.Mutated)),
                CustomMinimumSize = new Vector2(44, 44),
            };
            cell.AddThemeColorOverride("font_color", stoneColor);
            cell.AddThemeColorOverride("font_hover_color", stoneColor.Lightened(.18f));
            _inventory.AddChild(cell);
        }
        ...
        foreach (SkillChainDefinition chain in chains)
        {
            ...
            for (int index = 0; index < chain.TotalSockets; index++)
            {
                ...
                Color stoneColor = stone is null ? new Color("81786b") : SkillStoneVisual.ColorFor(stone.Definition.StableId);
                Texture2D? stoneIcon = stone is null ? null : _art?.SkillIcon(stone.Definition.StableId);
                var socket = new SkillSocketZone
                {
                    ...
                    ExpandIcon = true,
                    IconAlignment = HorizontalAlignment.Center,
                    TooltipText = stone is null ? $"空连接孔 {index + 1}\n从左侧技能石背包拖入" : SkillTooltip.Build(stone, $"{chain.DisplayName} · 孔 {index + 1}") + "\n右键卸下",
                    CustomMinimumSize = new Vector2(48, 44),
                };
                socket.AddThemeColorOverride("font_color", stoneColor);
                socket.AddThemeColorOverride("font_hover_color", stoneColor.Lightened(.18f));
                row.AddChild(socket);
    ...
    private Control BuildTargetSelector(GameSession session, SkillLinkConfiguration link)
    {
    ...
    private void AddTarget(Container row, ButtonGroup group, string text, SkillTargetPolicy policy, SkillTargetPolicy selected, GameSession session, string activeStoneId)
    {
...
public partial class SkillStoneCell : Button
{
    public string StoneInstanceId { get; set; } = string.Empty;
    public Color StoneColor { get; set; } = Colors.White;
    public override Variant _GetDragData(Vector2 atPosition)
    {
        SetDragPreview(SkillStoneVisual.DragPreview(Icon, Text, StoneColor));
        return Variant.From($"management-skill|{StoneInstanceId}");
    }
}

public partial class SkillSocketZone : Button
{
    public SkillStonePanel? Panel { get; set; }
    ...
    public override bool _CanDropData(Vector2 atPosition, Variant data) => data.VariantType == Variant.Type.String && data.AsString().StartsWith("management-skill|", StringComparison.Ordinal);
...
internal static class SkillStoneVisual
{
    public static Color ColorFor(string stableId)
    {
        uint hash = 2_166_136_261;
        foreach (char value in stableId) hash = unchecked((hash ^ value) * 16_777_619);
    ...
    public static Control DragPreview(Texture2D? icon, string glyph, Color color)
    {
        var panel = new PanelContainer
        {
            CustomMinimumSize = new Vector2(42, 42), Position = new Vector2(-20, -20),
