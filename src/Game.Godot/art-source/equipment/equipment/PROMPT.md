# Equipment 装备美术母版生成记录

- 生成模式：OpenAI 内置 `imagegen`。
- 风格参考：EquipmentArt 的武器、防具、配件与特殊职业母版，仅用于像素密度、轮廓、材质和调色方向。
...
核心提示词沿用 EquipmentArt：dark industrial fantasy action RPG、crisp hard-edged pixel art、strong dark outlines、restrained iron/bronze/leather/cloth/gem palette、one centered isolated icon per occupied cell、genuinely transparent background、no text/grid/border/watermark。

运行 `scripts/build_equipment_assets.ps1` 会按稳定目录顺序生成 13×19 的244底材图集和5×11的55传奇图集，并审计空格、边界接触和重复图标。
