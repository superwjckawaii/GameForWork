...
public partial class InformationWindow : IndependentWindow
{
    private Func<GameSession?>? _session;
    private RichTextLabel? _journey;

    public void Initialize(Func<GameSession?> session)
    {
        _session = session;
    ...
    private void RefreshJourney()
    {
        ...
        GameSession? session = _session?.Invoke();
        if (session is null)
        {
