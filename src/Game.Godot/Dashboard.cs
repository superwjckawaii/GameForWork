...
public partial class Dashboard : VBoxContainer
{
    ...
    private JewelStashPanel? _jewelStashPanel;
    ...
    private CraftingWorkbench? _craftingWorkbench;
    ...
    private CampaignRouteView? _campaignRoute;
    private WorldView? _worldView;
    private PassiveTreeView? _passiveTree;
    private ItemGrid? _storageGrid;
    private ItemGrid? _sortingGrid;
    private EquipmentPaperDoll? _equipmentGrid;
    private ItemGrid? _recoveryGrid;
    private ItemGrid? _buybackGrid;
    private ItemGrid? _heroLootGrid;
    private ItemGrid? _mercenaryLootGrid;
    private LootFilterPanel? _filterPanel;
    private SkillStonePanel? _skillStonePanel;
    private ExpeditionPanel? _expeditionPanel;
    private EquipmentCraftingPanel? _metalPanel;
    private TownPanel? _townPanel;
    private EndgamePanel? _endgamePanel;
    private AscendancyPanel? _ascendancyPanel;
    ...
    private CharacterKind _selectedCharacter;
    ...
    private JourneyDestination _pendingDestination;
    private JourneyDestination _warningDestination;
    ...
    public void AdvanceWorld(long realMilliseconds)
    {
        ...
        try
        {
            GameForWork.Core.Campaign.World.OfflineResult result = _session.AdvanceResponsive(realMilliseconds);
    ...
    private Control BuildCreationPanel()
    {
        ...
        ClassDefinition[] classes = ClassCatalog.All.ToArray();
        ...
        {
            ClassDefinition definition = classes[(int)index];
    ...
    private Control BuildFullPanel()
    {
        ...
        var characterTitleBar = new SecondaryTitleBar();
    ...
    private Control BuildOverviewPage()
    {
        ...
        AddButton(controls, "观察当前战斗", () => SetView(ViewMode.Active));
        AddButton(controls, "观察主角", () => SetView(ViewMode.Hero));
        AddButton(controls, "观察佣兵", () => SetView(ViewMode.Mercenaries));
    ...
    private Control BuildStoryPage()
    {
        ...
        _campaignRoute = new CampaignRouteView
        {
            SizeFlagsHorizontal = SizeFlags.ExpandFill,
            SizeFlagsVertical = SizeFlags.ExpandFill,
        };
    ...
    private Control BuildExpeditionPage()
    {
        ...
        {
            RequireSession().RecordJourneyEvent(JourneyEvent.ViewedCombatReport);
            _stateChanged?.Invoke();
            Refresh();
        };
        dispatch.AddChild(_expeditionPanel);
        var endgameTab = new VBoxContainer { Name = "异界与突破", SizeFlagsVertical = SizeFlags.ExpandFill };
        _endgamePanel = new EndgamePanel { SizeFlagsVertical = SizeFlags.ExpandFill, SizeFlagsHorizontal = SizeFlags.ExpandFill };
    ...
    private Control BuildCharacterItemsPage()
    {
        ...
        {
            _selectedCharacter = index == 0 ? CharacterKind.Hero : CharacterKind.Mercenary;
        ...
        _characterModes.TabChanged += index =>
        {
            ...
            {
                RequireSession().RecordJourneyEvent(JourneyEvent.InspectedSkills);
                _stateChanged?.Invoke();
                Refresh();
            }
        ...
        _jewelStashPanel = new JewelStashPanel { Name = "珠宝", SizeFlagsVertical = SizeFlags.ExpandFill };
        ...
        _equipmentGrid = new EquipmentPaperDoll();
    ...
    private Control BuildEquipmentMode()
    {
        ...
        _sortingGrid = BuildGrid(ItemContainerKind.SortingBag, 5, ManagementState.SortingBagCapacity, 36);
        ...
        _craftingWorkbench = new CraftingWorkbench
        {
        ...
        _batchScope.Select((int)BatchScope.Storage);
        ...
        AddButton(batch, "批量出售", () => ConfirmBatch(BatchAction.Sell));
        AddButton(batch, "批量分解", () => ConfirmBatch(BatchAction.Dismantle));
        AddButton(batch, "按连接数整理", () => SortItems(ItemSortMode.LinkedSockets));
        AddButton(batch, "按稀有度整理", () => SortItems(ItemSortMode.Rarity));
        AddButton(batch, "按物等整理", () => SortItems(ItemSortMode.ItemLevel));
        AddButton(batch, "按底材整理", () => SortItems(ItemSortMode.Base));
        AddButton(batch, "按最高T级整理", () => SortItems(ItemSortMode.HighestAffixTier));
        ...
        _buybackGrid = BuildGrid(ItemContainerKind.Buyback, 10, ManagementState.BuybackCapacity, 30);
    ...
    private Control BuildSkillMode()
    {
        ...
        _skillStonePanel = new SkillStonePanel();
    ...
    private Control BuildPassiveMode()
    {
        ...
        _passiveTree = new PassiveTreeView { MouseFilter = MouseFilterEnum.Stop };
        _passiveTree.SetAnchorsAndOffsetsPreset(Control.LayoutPreset.FullRect);
        _passiveTree.NodeSelected += stableId =>
        {
            PassiveNodeDefinition node = PassiveTree.Get(stableId);
            string selectedEffect = !string.IsNullOrWhiteSpace(node.SpecialRule)
                ? node.SpecialRule
                : string.Join("；", node.Effects.Select(UiText.PassiveEffect));
            _selectedPassive!.Text = $"已选择：{node.DisplayName} · {selectedEffect}";
            mastery!.Clear();
            IReadOnlyList<PassiveEffect> options = PassiveTree.MasteryOptions(node);
            IReadOnlyList<string> optionDescriptions = PassiveTree.MasteryOptionDescriptions(node);
        ...
        {
            if (_selectedCharacter != CharacterKind.Hero)
            {
        ...
        {
            bool changed = RequireSession().TrySocketBuildsJewel(stableId, instanceId, out string reason);
        ...
        {
            string? id = _passiveTree?.SelectedStableId;
            Changed(id is not null && RequireSession().TryUnsocketBuildsJewel(id) ? "珠宝已取下。" : "该棱孔没有珠宝。");
        });
        _heroOnlyControls.AddRange([chooseMastery, unsocketJewel]);
        overlay.AddChild(TreeHud(footer));
        _ascendancyPanel = new AscendancyPanel { Name = "升华", SizeFlagsVertical = SizeFlags.ExpandFill };
    ...
    private Control BuildTownPage()
    {
        ...
        _townPanel = new TownPanel { SizeFlagsVertical = SizeFlags.ExpandFill, SizeFlagsHorizontal = SizeFlags.ExpandFill };
        ...
        OptionButton legendary = AddOptions(workshop, "指定传奇", LegendaryDrops.ExchangePool.Select(item => item.DisplayName).ToArray());
        AddButton(workshop, $"{LegendaryDrops.PityMarkCost} 印记兑换", () =>
        {
            string id = LegendaryDrops.ExchangePool[legendary.Selected].StableId;
            Changed(RequireSession().TryExchangeLegendary(id) ? "指定传奇已存入仓库。" : "印记不足或仓库已满。");
        });
        loot.AddChild(new Label { Text = "掉落过滤器 · 有序规则（从上到下首次匹配）" });
        _filterPanel = new LootFilterPanel();
    ...
    private ItemGrid BuildGrid(ItemContainerKind kind, int columns, int capacity, float cellSize)
    {
        var grid = new ItemGrid { ContainerKind = kind };
    ...
    private void SelectCraftTarget(ItemContainerKind kind, int index, bool dragged = false)
    {
        ...
        if (kind is ItemContainerKind.Storage or ItemContainerKind.SortingBag or ItemContainerKind.Equipped)
        {
            ...
            if (_batchScope is not null && kind is ItemContainerKind.Storage or ItemContainerKind.SortingBag)
                _batchScope.Select(kind == ItemContainerKind.Storage ? (int)BatchScope.Storage : (int)BatchScope.SortingBag);
            Refresh();
        }
    }
    ...
    {
        ItemCommandService commands = ItemCommands();
    ...
    private void ConfirmDismantle(ItemCommandService commands)
    {
        ItemCommandResult initial = commands.Dismantle(_contextContainer, _contextIndex, confirmed: false);
    ...
    private void ConfirmBatch(BatchAction action)
    {
        ...
        BatchScope scope = (BatchScope)(_batchScope?.Selected ?? (int)BatchScope.Storage);
        bool includeCraftingBases = _batchIncludeCraftingBases?.ButtonPressed == true;
        BatchPreview preview = BatchItems.Preview(session, action, scope, maximum, includeCraftingBases, includeMythic);
        if (preview.Total == 0)
        {
        ...
        bool sell = action == BatchAction.Sell;
        ...
        {
            BatchExecution result = BatchItems.Execute(session, preview);
            Changed($"批量{(sell ? "出售" : "分解")}完成：成功 {result.Completed} 件，失败 {result.Failed} 件，受保护 {preview.Excluded} 件。");
        };
        _confirmDialog.PopupCentered();
    }

    private void SortItems(ItemSortMode mode)
    {
        GameSession session = RequireSession();
        BatchScope scope = (BatchScope)(_batchScope?.Selected ?? (int)BatchScope.Storage);
        if (scope is BatchScope.Storage or BatchScope.Both) session.World.Storage.Sort(mode);
        if (scope is BatchScope.SortingBag or BatchScope.Both) session.Management.SortSortingBag(mode);
        string modeName = mode switch
        {
            ItemSortMode.LinkedSockets => "连接数",
            ItemSortMode.Rarity => "稀有度",
            ItemSortMode.ItemLevel => "物品等级",
            ItemSortMode.Base => "底材",
            _ => "最高词缀T级",
        };
        Changed($"{(scope == BatchScope.Both ? "整理背包与仓库" : scope == BatchScope.Storage ? "仓库" : "整理背包")}已按{modeName}整理。");
    }
    ...
    private void Execute(ItemCommandResult result) => Changed(result.Message);
    ...
    private void RefreshJourneyInterface()
    {
        ...
        JourneyStepDefinition? current = session.Journey.CurrentStep;
    ...
    private void NotifyJourneyRewards(GameSession session)
    {
    ...
    private void RefreshUnlockedPages(GameSession session)
    {
        bool characterLocked = !session.Journey.TutorialAllowsPage(JourneyStep.EquipItem);
        ...
        SetHidden(_mainTabs!, _townPage!, !session.Journey.TutorialAllowsPage(JourneyStep.CraftItem, requireGateCompletion: true));
        ...
        if (_characterModes is not null)
        {
            ...
            SetHidden(_characterModes, _passiveMode!, !session.Journey.TutorialAllowsPage(JourneyStep.AllocatePassive));
            SetHidden(_characterModes, _aiMode!, !session.Journey.TutorialAllowsPage(JourneyStep.ConfigureSkillTarget, requireGateCompletion: true));
        }
    }

    private void RefreshStopWarning(GameSession session)
    {
        string text = string.Empty;
        JourneyDestination destination = JourneyDestination.Overview;
        if (session.Campaign.Defeated)
        {
            text = "主线战败：调整装备、技能或天赋后前往主线页继续。";
            destination = JourneyDestination.Story;
        }
                     ...
                     (link.SocketStoneInstanceIds?.Any(id => !string.IsNullOrEmpty(id)) ?? false)))
        {
            ...
            destination = JourneyDestination.Skills;
        }
        else
        {
            TeamExpeditionState? stopped = session.World.Teams.FirstOrDefault(team => team.IsStopped && team.StopReason != "manual_stop");
            if (stopped is not null)
            {
                text = $"{(stopped.Kind == ExpeditionTeamKind.Hero ? "主角" : "佣兵队")}远征已停止：{JourneyStopReason(stopped.StopReason)}。";
                destination = JourneyDestination.Expedition;
            }
        }
    ...
    {
        JourneyStepDefinition? current = RequireSession().Journey.CurrentStep;
        if (current is null)
        {
    ...
    private void ShowCompletionSummary()
    {
        ...
        DemoSummary summary = session.Journey.BuildSummary(session, hash);
        _completionDialog!.DialogText = BuildCompletionText(summary);
        _completionDialog.PopupCentered(new Vector2I(660, 540));
    }

    private void Navigate(JourneyDestination destination)
    {
        if (_mainTabs is null) return;
        bool characterDestination = destination is JourneyDestination.Equipment or JourneyDestination.Skills or
            JourneyDestination.Passives;
        Control? main = destination switch
        {
            JourneyDestination.Overview => _overviewPage!,
            JourneyDestination.Story => _storyPage!,
            JourneyDestination.Expedition => _expeditionPage!,
            JourneyDestination.Town => _townPage!,
            _ => null,
        };
        if (main is not null)
        {
        ...
        {
            JourneyDestination.Equipment => _equipmentMode,
            JourneyDestination.Skills => _skillMode,
            JourneyDestination.Passives => _passiveMode,
            _ => null,
        };
        if (mode is not null)
        {
    ...
    private static string BuildCompletionText(DemoSummary summary) =>
    ...
    private void Refresh()
    {
        ...
        if (characterActive)
        {
            ...
            TeamExpeditionState selectedTeam = _selectedCharacter == CharacterKind.Hero
                ? _session.World.Hero
                : _session.World.Mercenaries;
            EquipmentSummary equipment = selectedLoadout.CalculateSummary();
            MercenaryMember? selectedMercenary = SelectedMercenary();
            ...
            if (selectedMercenary is not null)
            {
                selectedSheet = CharacterBuildAssembler.Assemble(
                    ...
                    new SkillConfiguration(SkillIds.HeavyStrike, SkillSupport.Bleed)).Sheet;
            }
            _characterStatus!.Text = _selectedCharacter == CharacterKind.Hero
                ? $"{_session.Player.Name} · {ClassCatalog.Get(_session.Player.BaseClass).DisplayName} · Lv.{selectedTeam.Progression.Level}"
            ...
            Dictionary<string, string> jewelTooltips = _session.Jewels.Socketed.ToDictionary(
                pair => pair.Key,
                pair => UiText.JewelTooltip(_session.Jewels.Items.Single(jewel => jewel.InstanceId == pair.Value)),
            ...
            _bossFragmentsStatus!.Text =
                $"◆ 深渊监守者\n碎片 {_session.World.Expedition.AbyssWardenFragments}/{ExpeditionDirector.FragmentsPerTicket}　门票 ×{_session.World.Expedition.AbyssWardenTickets}\n" +
                $"下枚碎片进度 {_session.World.Expedition.MapsTowardNextFragment}/{ExpeditionDirector.MapsPerFragment}\n\n" +
                $"◆ 灰烬天垒\n碎片 {_session.Endgame.CitadelFragments}/{EndgameState.CitadelFragmentsPerTicket}　门票 ×{_session.Endgame.CitadelTickets}";
            _ascendancyPanel?.Refresh();
            bool heroSelected = _selectedCharacter == CharacterKind.Hero;
        ...
        _storyStatus!.Text = _session.Campaign.Completed
            ? "五幕主线已完成\n远征功能已开放。"
            : $"第 {currentNode!.Act} 幕 · {CampaignCatalog.ActNames[currentNode.Act - 1]}\n" +
    ...
    private static string BuildSummaryText(BuildSummary summary)
    {
        DefenseBreakdown defense = summary.Defense;
        OffenseBreakdown offense = summary.Offense;
        return
    ...
    private void RefreshPreviewSkillSelector()
    {
        ...
        string signature = string.Join('|', candidates.Select(item =>
            $"{item.StoneInstanceId}:{item.SkillId}:{item.Priority}:{string.Join(',', item.ExtendedBuildsSupports)}")) +
        ...
        {
            string name = ActiveSkillCatalog.ActiveForSkill(candidate.SkillId).Combat.DisplayName;
        ...
        _previewSkillSelector.Disabled = _selectedCharacter != CharacterKind.Hero || candidates.Count == 0;
    }

    private GameSession RequireSession() => _session ?? throw new InvalidOperationException("请先创建角色。");

    private ItemCommandService ItemCommands() => new(RequireSession(), _selectedCharacter, _selectedMercenaryId);

    private MercenaryMember? SelectedMercenary()
    {
        if (_session is null || _selectedCharacter != CharacterKind.Mercenary) return null;
        MercenaryMember? selected = _session.Town.Roster.FirstOrDefault(member => member.Identity.StableId == _selectedMercenaryId);
        return selected ?? _session.Town.Roster.FirstOrDefault();
    }

    private EquipmentLoadout SelectedLoadout() => _selectedCharacter == CharacterKind.Hero
    ...
    private void RefreshCharacterSelector()
    {
        ...
        foreach (MercenaryMember member in _session.Town.Roster)
            _characterSelector.AddItem($"佣兵：{member.Identity.Name}");
        int selectedIndex = _selectedCharacter == CharacterKind.Hero ? 0 :
        ...
        if (selectedIndex == 0) { _selectedCharacter = CharacterKind.Hero; _selectedMercenaryId = string.Empty; }
        else _selectedMercenaryId = _session.Town.Roster[selectedIndex - 1].Identity.StableId;
    }

    private static string MercenaryArchetypeName(GameForWork.Core.Town.MercenaryArchetype? archetype) => archetype switch
    {
        GameForWork.Core.Town.MercenaryArchetype.Guardian => "守卫",
        GameForWork.Core.Town.MercenaryArchetype.Ranger => "游猎者",
        GameForWork.Core.Town.MercenaryArchetype.Cantor => "颂仪者",
        GameForWork.Core.Town.MercenaryArchetype.Arcanist => "秘械师",
        _ => "未知职业",
    };
    ...
    private static string TeamText(TeamExpeditionState team, WorldState world) =>
    ...
    private static string EstimateFirstStop(TeamExpeditionState team, WorldState world)
    {
    ...
    private static string CompactTeam(TeamExpeditionState team) => team.ActiveMap is null
