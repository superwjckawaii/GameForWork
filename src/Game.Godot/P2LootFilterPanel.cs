...
public partial class P2LootFilterPanel : VBoxContainer
{
    ...
    private P2LootFilterEditorWindow? _editor;
    ...
    {
        _editor = new P2LootFilterEditorWindow { Theme = Theme };
        ...
        foreach (IGrouping<string, AffixDefinition> family in P1Affixes.All.Where(affix => affix.SourceId.Length > 0)
    ...
    private static string BaseName(string stableId) =>
        P1ItemBases.All.FirstOrDefault(item => item.StableId == stableId)?.DisplayName ?? stableId;
    ...
    private static string AffixFamilyLabel(IEnumerable<AffixDefinition> family)
    {
        ...
        string effect = IsAddedDamageRange(components)
            ...
            : string.Join(" + ", components.Select(component => P1UiText.ModifierName(component.Kind)).Distinct(StringComparer.Ordinal));
    ...
    private void ApplyPreset(int preset)
    {
        ...
        switch (preset)
        {
            case 0:
                ...
                _baseTier.Select((int)P29BaseTier.Pinnacle);
                break;
            case 1:
                _disposition!.Select((int)LootDisposition.Keep);
    ...
    private void RefreshEditorSummary()
    {
        ...
        var preview = new LootFilterRule("preview", (LootDisposition)_disposition.Selected,
            ...
            Category: EnumValue<ItemCategory>(_category!),
            BaseStableId: _base!.Selected <= 0 ? null : P1ItemBases.All.OrderBy(item => item.DisplayName).ElementAt(_base.Selected - 1).StableId,
            BaseTier: _baseTier!.Selected <= 0 ? null : (P29BaseTier)_baseTier.Selected,
...
public partial class P2LootFilterEditorWindow : P30IndependentWindow
{
