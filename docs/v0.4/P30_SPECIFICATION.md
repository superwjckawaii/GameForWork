...
- 中间计算使用 64 位整数，每个有明确语义的阶段结束时向下取整。
...
- 精神系与精神混合底材提供基础灵障，具体底材数值留到逐装备类型确认时统一决定。
...
护甲减伤先于物理抗性结算，护甲公式的分母必须使用尚未经过物理抗性和目标承伤修正的原始物理击中分量，避免物理抗性反过来增强护甲效率。转化后的伤害只使用当前最终类型对应的防御；已经转为火焰的物理来源伤害不再计算护甲或物理抗性。
...
击中顺序固定为：命中/闪避 → 转化与进攻修正 → 暴击 → 按当前类型汇总 → 格挡 → 物理分量护甲 → 各类型抗性 → 目标承伤提高/更多/更少 → 法术压制 → 守护类固定吸收 → 护盾 → 生命 → 以最终实际击中伤害计算异常。格挡成功不产生伤害型异常；压制后的异常以压制后的伤害为基数。
...
- 眩晕中断当前可打断动作。格挡伤害为 `0`，因此不会眩晕；法术压制后的实际伤害用于计算眩晕。
...
- 未命中、格挡、实际伤害为 `0`、持续伤害和超过目标剩余资源的过量击杀伤害不能产生偷取。
...
- 剩余部分生成普通偷取实例。普通瞬偷每次击中最多立即恢复对应最大资源的 `10%`，一秒内所有瞬偷合计最多恢复 `35%`；瞬偷不占普通偷取的每秒上限。
...
- 过量偷取不能让资源超过最大值，也不会自动转换成护盾；仍有有效实例时可以维持“偷取中”增益。
...
- 未命中、格挡、零伤害和持续伤害不触发。每种资源的击中回复合计上限为每秒最大资源 `50%`，并在角色面板显示；召唤物默认回复自身。
...
- 初始生命药剂最大 30 充能、消耗 10，3 秒恢复 `50%` 最大生命；初始法力药剂最大 30、消耗 10，3 秒恢复 `30%` 最大法力。高级底材和词缀延后逐装备类型确认。
...
逐类型的正式节点、连接和专精池统一记录在 [`P30_PASSIVE_CLUSTERS.md`](P30_PASSIVE_CLUSTERS.md)；当前剑类、斧类、锤类、匕首、弓类、法杖与符刃已确认，后续类型继续逐项追加。
...
- T1 武器局部核心词缀可提高该武器约 `15%～25%` 伤害；普通全局输出 T1 词缀对完整构筑约提升 `5%～10%`。
- 战功底材应强于同档普通底材，但必须存在装备槽和构筑机会成本；神话装备不进入标准平衡门槛。

最终预算会在逐天赋簇、逐珠宝、逐装备类型确认时形成明确数据表。

## 16. 36 套固定构筑和自动审计

- 十八个升华各维护开荒与终局两套完整构筑，共 36 套。
- 每套必须固定职业、等级、主天赋路径、专精、珠宝、8 点升华、装备及词缀、孔组、主动/辅助技能、药剂和 AI 目标规则。
- 开荒基准使用 70 级、无传奇、4 连和普通稀有装备；终局基准使用 100 级、6 连和高阶稀有装备，允许 0～2 件构筑传奇或战功底材，不使用神话装备。
- T20 正式门禁使用 100 级角色；120 级仅作为最终突破后的强度上限审计。
...
3. 装备词缀：逐装备类型确认局部/全局词缀、T 级、数值、权重和标签。
...
- 各武器类型天赋簇的节点名称、数值、连接和专精选项。
- 珠宝目录及掉落。
- 各装备类型正式词缀表。

## 19. 实施边界

本文件全部待确认项关闭后，再统一开始 P30 代码实施。实施完成后运行 Debug/Release 测试、Godot 启动检查、完整公式测试、固定种子构筑审计和必要的性能检查，通过后直接推送。讨论期间只提交设计文档，不提前混入半套战斗实现。
