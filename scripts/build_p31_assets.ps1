...
$sourcePath = Join-Path $RepositoryRoot 'src\Game.Godot\art-source\p31\imagegen\p31-vfx-master.png'
$assetRoot = Join-Path $RepositoryRoot 'src\Game.Godot\assets\p31'
$vfxRoot = Join-Path $assetRoot 'vfx'
$outputPath = Join-Path $vfxRoot 'p31-combat-vfx.png'
$manifestPath = Join-Path $assetRoot 'p31-assets.json'
if (-not (Test-Path -LiteralPath $sourcePath)) { throw "Missing P31 VFX source: $sourcePath" }
...
$manifest = [ordered]@{
    version = '0.4.0'
    source = 'art-source/p31/imagegen/p31-vfx-master.png'
    generated = 'assets/p31/vfx/p31-combat-vfx.png'
    grid = '4x4'
    cell = '64x64'
...
Write-Host "[p31-assets] generated=$outputPath sha256=$hash"
