...
$sourceRoot = Join-Path $RepositoryRoot 'src\Game.Godot\art-source\presentation\imagegen\trees'
$assetRoot = Join-Path $RepositoryRoot 'src\Game.Godot\assets\presentation\trees'
$ascendancyRoot = Join-Path $assetRoot 'ascendancy'
New-Item -ItemType Directory -Force -Path $assetRoot, $ascendancyRoot | Out-Null

$layoutPath = Join-Path ([System.IO.Path]::GetTempPath()) 'gameforwork-presentation-tree-layout.json'
$exportProject = Join-Path $RepositoryRoot 'tools\ArtTreeExport\ArtTreeExport.csproj'
& dotnet run --project $exportProject --configuration Release --no-launch-profile -- $layoutPath | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Presentation tree layout export failed.' }
...
function Save-Passive {
    ...
    try {
        Draw-RotatedBase $graphics (Join-Path $sourceRoot 'presentation-passive-tree-master.png') $size $size
        ...
        $bitmap.Save((Join-Path $assetRoot 'presentation-passive-backdrop.png'), [System.Drawing.Imaging.ImageFormat]::Png)
...
function Save-Atlas {
    ...
    $graphics = New-Graphics $bitmap
    try {
        Draw-Base $graphics (Join-Path $sourceRoot 'presentation-atlas-tree-master.png') $size $size
        ...
        $bitmap.Save((Join-Path $assetRoot 'presentation-atlas-backdrop.png'), [System.Drawing.Imaging.ImageFormat]::Png)
    } finally { $graphics.Dispose(); $bitmap.Dispose() }
}
...
function Save-Ascendancies {
    ...
    for ($index = 0; $index -lt 18; $index++) {
        ...
        try {
            ...
            $destination = Join-Path $ascendancyRoot ("presentation-ascendancy-{0:D2}-{1}.png" -f ($index + 1), $names[$index])
...
Save-Atlas
Save-Ascendancies
Write-Host '[presentation-tree-assets] generated main passive, atlas and 18 ascendancy backdrops from exact runtime coordinates.'
