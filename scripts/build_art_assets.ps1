...
$monstersSourceRoot = Join-Path $RepositoryRoot 'src\Game.Godot\art-source\monsters\imagegen'
...
if (-not $SkipAnimations) {
    ...
    Build-AnimationAtlas -Sprites $bossSprites -Destination (Join-Path $assetRoot 'enemies\art-boss-animation.png') -CellWidth 72 -CellHeight 80 -Padding 8
...
Build-TownPreview -Source $townSource -Destination (Join-Path $assetRoot 'town\art-town-district.png')
Build-GridAtlas -Source $townSource -SourceColumns 4 -SourceRows 2 -SourceIndices (0..6) `
    -Destination (Join-Path $assetRoot 'town\art-building-atlas.png') -Columns 4 -CellWidth 160 -CellHeight 120 -Padding 3
$townSource.Dispose()

Build-MetalAtlas -Destination (Join-Path $assetRoot 'ui\art-metal-atlas.png')
& (Join-Path $PSScriptRoot 'build_animation_assets.ps1') -RepositoryRoot $RepositoryRoot
if ($LASTEXITCODE -ne 0) { throw "Art.1 asset build failed with exit code $LASTEXITCODE." }
...
$manifest | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $assetRoot 'art-assets.json') -Encoding utf8
Write-Host "Art assets generated at $assetRoot"
