...
$equipmentArtSourceRoot = Join-Path $RepositoryRoot 'src\Game.Godot\art-source\equipmentArt\equipment'
$equipmentSourceRoot = Join-Path $RepositoryRoot 'src\Game.Godot\art-source\equipment\equipment'
$outputRoot = Join-Path $RepositoryRoot 'src\Game.Godot\assets\equipmentArt\ui'
$equipmentPath = Join-Path $outputRoot 'equipmentArt-equipment-atlas.png'
$legendaryPath = Join-Path $outputRoot 'equipmentArt-legendary-atlas.png'

$sourcePaths = @{
    legacyWeapons = Join-Path $equipmentArtSourceRoot 'master-weapons.png'
    legacyArmor = Join-Path $equipmentArtSourceRoot 'master-armor.png'
    legacyAccessories = Join-Path $equipmentArtSourceRoot 'master-accessories.png'
    legacyRanged = Join-Path $equipmentArtSourceRoot 'master-special-ranged.png'
    legacyClass = Join-Path $equipmentArtSourceRoot 'master-special-class.png'
    equipmentWarfront = Join-Path $equipmentSourceRoot 'master-warfront-and-core.png'
    equipmentSpirit = Join-Path $equipmentSourceRoot 'master-spirit-barrier.png'
    equipmentWeapons = Join-Path $equipmentSourceRoot 'master-weapons.png'
    equipmentSpecialWeapons = Join-Path $equipmentSourceRoot 'master-special-weapons.png'
    equipmentSpiritShields = Join-Path $equipmentSourceRoot 'master-spirit-shields.png'
    equipmentEvasionShields = Join-Path $equipmentSourceRoot 'master-evasion-spirit-shields.png'
    equipmentLegendary = Join-Path $equipmentSourceRoot 'master-legendary.png'
}
...
try {
    for ($index = 0; $index -lt $bases.Count; $index++) {
        ...
        if ($index -lt 20) {
            ...
            $source = $sources.equipmentWarfront
            ...
            if ($item.id -notmatch '^equipment\.base\.(.+)\.(\d+)$') { throw "Unexpected Archetypes art ID $($item.id)" }
            $key = $Matches[1]; $variant = [int]$Matches[2]; $mapping = $archetypesGroups[$key]; if ($null -eq $mapping) { throw "Missing Archetypes art group $key" }
            $source = $sources[$mapping[0]]; $cell = Get-CellRectangle $source $mapping[1] $mapping[2][$variant - 1] 5 6
        } elseif ($index -lt 200) {
            if ($index -in @(191,195,199)) { $source=$sources.equipmentSpiritShields; $column=[Array]::IndexOf([object[]]@(191,195,199),$index); $cell=Get-CellRectangle $source $column 0 3 1 }
            elseif ($index -ge 192 -and $index -le 194) { $source=$sources.equipmentEvasionShields; $cell=Get-CellRectangle $source ($index-192) 0 3 1 }
            else {
                $source=$sources.equipmentSpirit
            ...
            if ($index -lt 228) { $source=$sources.equipmentWeapons; $column=[Math]::Floor(($index-200)/4); $row=($index-200)%4; $columns=7 }
            else { $source=$sources.equipmentSpecialWeapons; $column=[Math]::Floor(($index-228)/4); $row=($index-228)%4; $columns=4 }
    ...
    try {
        for ($index = 0; $index -lt $legendaries.Count; $index++) {
            ...
            if ($index -lt 25) {
                ...
                $source=$sources.equipmentLegendary; $generated=$index-25; $cell=Get-CellRectangle $source ($generated%6) ([Math]::Floor($generated/6)) 6 5
